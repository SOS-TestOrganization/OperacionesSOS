...
* Desarrollado por		: <\A Ing. Samuel Mu±oz							A\> 
* Descripci=n			: <\D									D\> 
* Observaciones			: <\O									O\> 
* Parßmetros			: <\P  									P\> 
* Variables			: <\V  									V\> 
* Fecha Creaci=n		: <\FC 2003/00/00  							FC\> 
* 
*--------------------------------------------------------------------------------------------------------------------------------------- 
...
* Descripci=n			: <\DM									DM\> 
* 				: <\DM									DM\> 
* Nuevos Parßmetros		: <\PM								> 
* Nuevas Variables		: <\VM  									VM\> 
* Fecha Modificaci=n		: <\FM									FM\> 
*-----------------------------------------------------------------------------------------------------------------------------------*/ 
--Exec spPmConsultaAfiliado_AlDia 1,'94400802',1,'','','' 
CREATE procedure spRecobrosConsultaAfiliado_AlDia
--Declare
...
-- Se crea funci=n para calcular la fecha hasta para los reportes de validador de derechos, requerimiento segun gerencia, antes a la fecha del sistema se sumaba 5 dfas, ahora son 115 
...
Begin 
	...
	From	dbo.tbBeneficiariosValidador b INNER JOIN dbo.tbContratosValidador c On 
		b.nmro_cntrto			= c.nmro_cntrto 
	...
	if Exists(Select 1 From @tb_tmpAfiliado Where @fcha_vldccn Between Convert(char(10),inco_vgnca_bnfcro,111) And Convert(char(10),fn_vgnca_bnfcro,111)) 
	Begin 
...
If (select count(*) from  @tb_tmpAfiliado) = 0
Begin
	...
	Begin 
		...
		fcha_dsde,			fcha_hsta, 
		tlfno_rsdnca,			drccn_rsdnca 
		From	dbo.tbBeneficiariosFormularioValidador 
...
If (select count(*) from @tb_tmpAfiliado) = 0
Begin
--smns_aflcn_ss_ps Estas son las semanas de afiliaci=n al sistema de seguridad 
...
Begin
	...
	If Isnull(@Bd,'') ='' Or @bd = '4' 
	Begin 

		Insert Into @tb_tmpAfiliado( 
			prmr_aplldo, 			sgndo_aplldo, 			prmr_nmbre,  
			sgndo_nmbre, 			cnsctvo_cdgo_tpo_idntfccn,	nmro_idntfccn, 
			fcha_ncmnto, 			cnsctvo_cdgo_sxo, 		cnsctvo_cdgo_prntsco,	
			cnsctvo_cdgo_pln,		cnsctvo_cdgo_tpo_afldo,		cnsctvo_cdgo_rngo_slrl,	
			inco_vgnca_bnfcro,		fn_vgnca_bnfcro, 		smns_aflcn_eps_ps,	
			smns_aflcn_ss_ps,		smns_aflcn_eps_pc, 		smns_aflcn_ss_pc,	
			cnsctvo_cdgo_estdo_drcho,	cdgo_ips_prmra, 				
			cnsctvo_cdgo_tpo_cntrto, 	nmro_cntrto, 			orgn,
			fcha_dsde,			fcha_hsta,			smns_ctzds_ss_ps,		
			smns_ctzds_eps_ps
			) 
		 
		
		Select	ae.prmr_aplldo,			ae.sgndo_aplldo,		ae.prmr_nmbre,
			ae.sgndo_nmbre,			ae.cnsctvo_cdgo_tpo_idntfccn,	ae.nmro_idntfccn,
			ae.fcha_ncmnto,			ae.cnsctvo_cdgo_sxo,		ae.cnsctvo_cdgo_prntsco,
			ae.cnsctvo_cdgo_pln,		ae.cnsctvo_cdgo_tpo_afldo,	ae.cnsctvo_cdgo_rngo_slrl,
			ae.inco_vgnca,			ae.fn_vgnca,			0,
			0,				0,				ae.smns_ctzds,
			0,				ae.cdgo_ips_prmra,				
			0,				'000000000000000',		'4',
			ae.inco_vgnca,			ae.fn_vgnca,			0,
			ae.smns_ctzds
		FROM bdSisalud.dbo.tbInfAfiliadosEspeciales ae   
		INNER JOIN bdAfiliacionValidador.dbo.tbPlanes c  
			ON ae.cnsctvo_cdgo_pln		= c.cnsctvo_cdgo_pln  
		INNER JOIN bdSisalud.dbo.tbSexoSalud d  
			ON ae.cnsctvo_cdgo_sxo		= d.cnsctvo_cdgo_sxo  
		INNER JOIN bdAfiliacionValidador.dbo.tbCiudades_Vigencias h  
			ON ae.cnsctvo_cdgo_cdd 		= h.cnsctvo_cdgo_cdd  
		WHERE 	ae.cnsctvo_cdgo_tpo_idntfccn 	= @cnsctvo_cdgo_tpo_idntfccn  
		AND 	ae.nmro_idntfccn 		= @nmro_idntfccn 
...
If @bd Is Null 
Begin 
	Set @bd 		= '6'  -- No existe en el sistema 
	Set @mnsje 		= 'NO EXISTE EN EL SISTEMA' 
...
Begin 
	...
	From	bdSisalud..tbDetalleEapb_Vigencias a, @tb_tmpAfiliado b  
	Where	a.cdgo_eapb	= b.cdgo_eapb 
	And	orgn		= '3' 
...
Begin 
	...
	Select	b.smns_ctzds,b.smns_aflcn,b.smns_ctzds_antrr_eps,smns_aflcn_antrr_eps, 
		...
		b.inco_vgnca_bnfcro,b.fn_vgnca_bnfcro 
	From	@tb_tmpAfiliado t Inner Join dbo.tbBeneficiariosValidador b 
	On	b.nmro_unco_idntfccn_afldo	= t.nmro_unco_idntfccn_afldo 
	...
	Begin 
		...
		Begin 
			...
			From	@tb_tmpAfiliado t,	@tiempoAfiliacion b, 
				...
				From	@tiempoAfiliacion 
				Where	cnsctvo_cdgo_tpo_cntrto	= 1) as g 
			Where	b.cnsctvo_cdgo_tpo_cntrto	= 1 
	...
	Begin 
		...
		From	@tb_tmpAfiliado t,	@tiempoAfiliacion b 
			Inner Join dbo.tbContratosValidador c 
	...
	From	@tb_tmpAfiliado t INNER JOIN dbo.TbMatrizDerechosValidador m 
		With(index(IX_TbMatrizDerechosValidador_inco_fin_v)) on
	...
	And	m.cnsctvo_bnfcro		= t.cnsctvo_bnfcro 
				 INNER JOIN dbo.tbCausasDerechoValidador c On 
...
And	cnsctvo_cdgo_estdo_drcho	= 7	--Retirado 
...
--Cuando el cnsctvo_tpo_vldcn_actva = 0 se debe inactivar las validaciones. 
Update	@tb_tmpAfiliado 
Set	cnsctvo_tpo_vldcn_actva		= 0 
Where	orgn				= '1'	--Contratos 
And	Convert(char(10),inco_vgnca_bnfcro,111) > @fcha_vldccn 
And	cnsctvo_cdgo_estdo_drcho	= 6	--En Proceso 
...
*/ 
--Select	Top 1 
Select	prmr_aplldo,		sgndo_aplldo, 
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spRecobrosConsultaAfiliado_AlDia] TO [Coordinador Parametros Recobro]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spRecobrosConsultaAfiliado_AlDia] TO [Liquidador Recobros]
    AS [dbo];
