...
*---------------------------------------------------------------------------------*/
CREATE  PROCEDURE SpGrabarNotaCreditoImpuestos
...
IF	@lnTipoAplicacion	=	1		-- si para aplicar ahora
	Begin
		...
		select  @lnValorTotalNota 	= sum(vlr_sn_iva) 	from #TMPcontratosresponsable
		...
		Where	valor	>	0
 		...
 		select 	@Valor_iva_nota 	= sum(vlr_iva) 		from #TMPcontratosresponsable
		Where	valor	>	0
...
if	@lnTipoAplicacion	=	1		-- si para aplicar ahora
	...
	IF	 @lntipoDocumento		=	9   -- Estado de cuenta
		begin	
			...
			Select		@NuevoSaldoEstadoCuenta		=	sldo_estdo_cnta	-		(@Valor_saldo_nota+@Valor_iva_nota)
			From		bdcarteraPac.dbo.tbestadoscuenta
			Where		cnsctvo_estdo_cnta		=	@cnsctvo_estdo_cnta

			Update	bdcarteraPac.dbo.tbestadoscuenta
			Set	sldo_estdo_cnta			=	sldo_estdo_cnta		-		(@Valor_saldo_nota+@Valor_iva_nota)
			Where	cnsctvo_estdo_cnta		=	@cnsctvo_estdo_cnta
	...
	IF	 @lntipoDocumento		=	10   -- Nota debito
		begin	
			...
			Set	sldo_nta_cntrto		=	a.sldo_nta_cntrto - (tmpNotaCredito.vlr+tmpNotaCredito.vlr_iva) ,----OJO 
				fcha_ultma_mdfccn	=	@ldFechaSistema,
				usro_ultma_mdfccn	=	@lcUsuario
			...
			--Se verifica el nuevo saldo
			Select	@NuevoSaldoEstadoCuenta		=	sldo_nta	-	(@Valor_saldo_nota+@Valor_iva_nota)
			...
			Set	sldo_nta			=	sldo_nta	-	(@Valor_saldo_nota+@Valor_iva_nota)
			Where	ltrim(rtrim(nmro_nta))		=	ltrim(rtrim(convert(varchar(15),@cnsctvo_estdo_cnta)))
			And	cnsctvo_cdgo_tpo_nta		=	1
