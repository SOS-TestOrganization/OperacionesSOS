...
/*---------------------------------------------------------------------------------
* Metodo o PRG 		:  SpDesaplicarNotaCreditoREI
* Desarrollado por		: <\A Ing. Andres Camelo (illustrato ltda) 								A\>
* Descripcion			: <\D Este procedimiento  permite desaplicar el pago de una nota credito REI y cambiar de estado a la  nota Reintegro asociada	D\>
* Observaciones			: <\O  													O\>
* Parametros			: <\P													P\>
* Variables			: <\V  													V\>
...
*---------------------------------------------------------------------------------*/
CREATE  PROCEDURE [dbo].[SpDesaplicarNotaCreditoREI]
...
@numeroNotaReintegro            Varchar(15),
@lcTipoNotaReintegro            udtConsecutivo
...
If  (@lnConsecutivoEstadoActualNota	=	5	or 	@lnConsecutivoEstadoActualNota	=	6) -- sin aplicar o anulada
	Begin 
		...
		Set	@lcMensaje		=	'No se puede desaplicar la nota REI su estado no lo permite'
		Return -1
	end	
...
--Actualiza el estado de las notas debito a ingresado si los saldo son iguales 
...
If  @@error!=0  
	Begin 
		Set	@lnProcesoExitoso	=	1
		Set	@lcMensaje		=	'Error Actualizando el estado de la nota 1'
		Rollback tran 
		Return -1
	end	


--Actualiza el estado de las notas debito a cancelado parcial si los saldos son diferentes 
Update  bdcarteraPac.dbo.TbNotasPac
Set	cnsctvo_cdgo_estdo_nta	=	2
From	bdcarteraPac.dbo.TbNotasPac a inner join  #tmpNotasAplicadas b
	on (	a.nmro_nta		=	b.nmro_nta_aplcda
	And	a.cnsctvo_cdgo_tpo_nta	=	b.cnsctvo_cdgo_tpo_nta_aplcda)
Where	a.vlr_nta +   a.vlr_iva  	!=	    a.sldo_nta
...
If  @@error!=0  
	Begin 
		Set	@lnProcesoExitoso	=	1
		Set	@lcMensaje		=	'Error Actualizando el estado de la nota 2'
		Rollback tran 
		Return -1
	end	
...
Set	sldo_nta				=	sldo_nta  + @lntotalValorEstadosCuenta + @lnTotalValorNotasDebito ,
	cnsctvo_cdgo_estdo_nta		=	6	-- ANULADA
From	bdcarteraPac.dbo.TbnotasPac a 
Where 	a.nmro_nta			= 	@nmro_nta
And	a.cnsctvo_cdgo_tpo_nta		=	@lcTipoNota
...
-- consultamos la nota reintegro asociado a la nota credito REI
select @numeroNotaReintegro = nmro_nta_rntgro,
       @lcTipoNotaReintegro = cnsctvo_cdgo_tpo_nta_rntgro
from   tbRelacionNotasCreditoReiNotasReintegro
where  nmro_nta_ri              =  @nmro_nta
and    cnsctvo_cdgo_tpo_nta_ri  =  @lcTipoNota


if   @numeroNotaReintegro > 0 and @lcTipoNotaReintegro > 0
  begin

  --- desaplicamos el reintegro si esta en estado aplicado
    select
    @lcTipoNotaReintegro       =  cnsctvo_cdgo_tpo_nta,
    @numeroNotaReintegro       =  nmro_nta,
    ...
    from TbnotasPac with (nolock)
	where nmro_nta = @numeroNotaReintegro 
    and cnsctvo_cdgo_tpo_nta = @lcTipoNotaReintegro
    and cnsctvo_cdgo_estdo_nta = 4 

    exec SpDesaplicarNotaCreditoReintegro @lcTipoNotaReintegro,@numeroNotaReintegro,@nmro_unco_idntfccn_empldr,@cnsctvo_scrsl,@cnsctvo_cdgo_clse_aprtnte,@lcUsuario,null,null


      -- marcamos que el reintegro como desaplicado
   update tbRelacionNotasCreditoReiNotasReintegro set rntgro_aplcdo = 'N'
   where  nmro_nta_ri              =  @nmro_nta
   and    cnsctvo_cdgo_tpo_nta_ri  =  @lcTipoNota
