...
/*---------------------------------------------------------------------------------
* Metodo o PRG		:  spEjecutaConsSaldosFavorxEmpleador
* Desarrollado por	: <\A Ing. Fernando Valencia E 				A\>
* Descripcion		: <\D Este procedimiento  permite listar saldos a favor por Empleador		D\>
* Observaciones		: <\O  								O\>
* Parametros		: <\P								P\>
* Variables		: <\V  								V\>
* Fecha Creacion	: <\FC 2008/01/10						FC\>
**---------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[spEjecutaConsSaldosFavorxcontrato]
...
Update  a 
Set nombre_afiliado  =   ltrim(rtrim(prmr_nmbre))  + ' ' + ltrim(rtrim(sgndo_nmbre)) + ' ' + ltrim(rtrim(prmr_aplldo)) + ' '+ ltrim(rtrim(sgndo_aplldo)),
...
tipo_id_empleador as [Tipo_Id_Empleador], 
Numero_id_Empleador as [Numero_Id_Empleador] ,
nombre_empleador as [Nombre_Responsable], 
nmro_estdo_cnta as [Numero_Estado_Cuenta], 
nmro_cntrto as [Numero_Contrato],
sldo_fvr as [Saldo_Favor],
vlr_pgo as [Valor_Pago],
fcha_aplccn as [Fecha_Aplicacion] ,  
fcha_rcdo as [Fecha_Recaudo],
nmro_pgo as [Numero_Pago]
