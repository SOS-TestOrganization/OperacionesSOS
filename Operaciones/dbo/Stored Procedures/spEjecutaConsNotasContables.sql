...
/*---------------------------------------------------------------------------------
* Metodo o PRG   : spEjecutaConsNotasContables
...
*---------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[spEjecutaConsNotasContables]
...
( nmro_nta								varchar(15),
  ...
  fcha_dcmnto_dbto						Datetime,
  cnsctvo_estdo_cnta_cntrto				Int,
  tpo_dcmnto_dbto						Char(2)
  )
...
if (@cnsctvo_cdgo_tpo_nta != 4 or @cnsctvo_cdgo_tpo_nta is null)
...
-- primero se hace para estados de cuenta
 ...
 from tbnotasPac     a, 
  ...
  bdconsulta..tbSucursalesAportante  b  ,
  tbnotasContrato     c,
  tbtiposNotas     d ,
  bdconsulta..tbcontratos  ct,
...
And (@nmro_nta_ini is null or (@nmro_nta_ini is not null and convert(int,a.nmro_nta)  between convert(int,@nmro_nta_ini) and convert(int,@nmro_nta_fn)))
 ...
 -- Se buca el perido del ESTADO DE CUENTA que afecto la nota credito

 Update	a
 Set	cnsctvo_cdgo_prdo_lqdcn		= d.cnsctvo_cdgo_prdo_lqdcn,
		tpo_dcmnto_dbto				= 'EC'
 -- Select a.*
 From	#tmpNotasPac a , 
		...
		tbEstadosCuenta c,
		tbLiquidaciones d
 Where  a.cnsctvo_cdgo_tpo_nta		!= 1		-- NOTA DEBITO
 And	a.cnsctvo_estdo_cnta_cntrto = b.cnsctvo_estdo_cnta_cntrto
 And	b.cnsctvo_estdo_cnta		= c.cnsctvo_estdo_cnta
 And	c.cnsctvo_cdgo_lqdcn		= d.cnsctvo_cdgo_lqdcn

 
 Update a
 Set	fcha_dcmnto_dbto				= Convert(Char(10),b.fcha_incl_prdo_lqdcn,111)
 from #tmpNotasPac a
 Inner Join  bdCarteraPac.dbo.tbPeriodosliquidacion_Vigencias b		On b.cnsctvo_cdgo_prdo_lqdcn   = a.cnsctvo_cdgo_prdo_lqdcn


 -- Se buca el perido de la NOTA DEBITO que afecto la nota credito

  Update	a
 Set	fcha_dcmnto_dbto				= Convert(Char(10),c.fcha_crcn_nta,111),
		tpo_dcmnto_dbto					= 'ND'
 -- Select a.*
 From	#tmpNotasPac a ,
		tbNotasAplicadas b,
 ...
 And	b.nmro_nta_aplcda				= c.nmro_nta
 And	b.cnsctvo_cdgo_tpo_nta_aplcda	= c.cnsctvo_cdgo_tpo_nta
...
/* Select a.*  , @Fecha_minima  fecha_minima ,@Fecha_maxima  Fecha_Maxima
 ...
 Order by  cnsctvo_cdgo_sde, cnsctvo_cdgo_pln, fcha_crcn_nta, cnsctvo_cdgo_tpo_nta,
  ...
  Select a.* from tbnotasPac a, #tmpNotasPac b Where a.nmro_nta = b.nmro_nta
