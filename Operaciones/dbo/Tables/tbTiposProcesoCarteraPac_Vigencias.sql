CREATE TABLE [dbo].[tbTiposProcesoCarteraPac_Vigencias] (
    [cnsctvo_Vgnca_tpo_prcso] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_tpo_prcso]  [dbo].[udtConsecutivo] NULL,
    [cdgo_tpo_prcso]          [dbo].[udtCodigo]      NULL,
    [Dscrpcn_prcso]           [dbo].[udtDescripcion] NULL,
    ...
    CONSTRAINT [PK_tbTiposProcesoTesoreria_Vigencias] PRIMARY KEY CLUSTERED ([cnsctvo_Vgnca_tpo_prcso] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbTiposProcesoCarteraPac_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbTiposProcesoCarteraPac_Vigencias_tbTiposProcesoCarteraPac] FOREIGN KEY ([cnsctvo_cdgo_tpo_prcso]) REFERENCES [dbo].[tbTiposProcesoCarteraPac] ([cnsctvo_cdgo_tpo_prcso]) ON DELETE CASCADE
);


GO
CREATE STATISTICS [cnsctvo_cdgo_tpo_prcso]
    ON [dbo].[tbTiposProcesoCarteraPac_Vigencias]([cnsctvo_cdgo_tpo_prcso], [cnsctvo_Vgnca_tpo_prcso]);


GO
CREATE STATISTICS [cdgo_tpo_prcso]
    ON [dbo].[tbTiposProcesoCarteraPac_Vigencias]([cdgo_tpo_prcso], [cnsctvo_Vgnca_tpo_prcso]);


GO
CREATE STATISTICS [Dscrpcn_prcso]
    ON [dbo].[tbTiposProcesoCarteraPac_Vigencias]([Dscrpcn_prcso], [cnsctvo_Vgnca_tpo_prcso]);


GO
CREATE STATISTICS [inco_vgnca]
    ON [dbo].[tbTiposProcesoCarteraPac_Vigencias]([inco_vgnca], [cnsctvo_Vgnca_tpo_prcso]);


GO
CREATE STATISTICS [fn_vgnca]
    ON [dbo].[tbTiposProcesoCarteraPac_Vigencias]([fn_vgnca], [cnsctvo_Vgnca_tpo_prcso]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbTiposProcesoCarteraPac_Vigencias]([fcha_crcn], [cnsctvo_Vgnca_tpo_prcso]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbTiposProcesoCarteraPac_Vigencias]([usro_crcn], [cnsctvo_Vgnca_tpo_prcso]);


GO
CREATE STATISTICS [obsrvcns]
    ON [dbo].[tbTiposProcesoCarteraPac_Vigencias]([obsrvcns], [cnsctvo_Vgnca_tpo_prcso]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbTiposProcesoCarteraPac_Vigencias]([vsble_usro], [cnsctvo_Vgnca_tpo_prcso]);


GO
CREATE STATISTICS [tme_stmp]
    ON [dbo].[tbTiposProcesoCarteraPac_Vigencias]([tme_stmp]);


GO

/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbTiposProcesoCarteraPac_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbTiposProcesoCarteraPac_Vigencias] ON   [dbo].[tbTiposProcesoCarteraPac_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_tpo_prcso	,
		@cdgo_ara = cdgo_tpo_prcso		,
		@cdgo_vgnca	= cnsctvo_vgnca_tpo_prcso,
		@dscrpcn =  dscrpcn_prcso
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbTiposProcesoCarteraPac_Vigencias p
	...
	And	p.cnsctvo_cdgo_tpo_prcso = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_prcso =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_tpo_prcso !=  @cdgo_vgnca	
...
update tbTiposProcesoCarteraPac set dscrpcn_prcso =  @dscrpcn
where cnsctvo_cdgo_tpo_prcso = @Codigo_Consecutivo
...
GO
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbTiposProcesoCarteraPac_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbModosAplicacion_Vigencias 		D\>
* 			  <\D Cuando se actualiza un Campo de  tbTiposProcesoCarteraPac_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbTiposProcesoCarteraPac_Vigencias]   On [dbo].[tbTiposProcesoCarteraPac_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbTiposProcesoCarteraPac_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbTiposProcesoCarteraPac_Vigencias',@lcCampo,
						d.cnsctvo_Vgnca_tpo_prcso, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_prcso
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_prcso = n.cnsctvo_cdgo_tpo_prcso AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_Vgnca_tpo_prcso, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_prcso
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_prcso = n.cnsctvo_cdgo_tpo_prcso AND 
							(
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbTiposProcesoCarteraPac_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbTiposProcesoCarteraPac_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbTiposProcesoCarteraPac_Vigencias] ON   [dbo].[tbTiposProcesoCarteraPac_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbTiposProcesoCarteraPac_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_tpo_prcso
from INSERTED
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_tpo_prcso = n.cnsctvo_vgnca_tpo_prcso)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbTiposProcesoCarteraPac_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbTiposProcesoCarteraPac y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbTiposProcesoCarteraPac_Vigencias  y  ' + @lcopcion_menu	
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposProcesoCarteraPac_Vigencias
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposProcesoCarteraPac_Vigencias]
ON  [dbo].[tbTiposProcesoCarteraPac_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposProcesoCarteraPac_Vigencias ','cdgo_tpo_prcso')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_tpo_prcso = @lcCodigov  Or  cdgo_tpo_prcso =  @lcCodigoi)
Begin
	RollBack Transaction
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbTiposProcesoCarteraPac_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbTiposProcesoCarteraPac_Vigencias] ON   [dbo].[tbTiposProcesoCarteraPac_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_tpo_prcso,
	      @cnsctvo_vgnca = cnsctvo_vgnca_tpo_prcso			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_prcso = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_tpo_prcso != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposProcesoCarteraPac_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposProcesoCarteraPac_Vigencias_usro_crcn]    ON    [dbo].[tbTiposProcesoCarteraPac_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
