CREATE TABLE [dbo].[tbEstadosEstadosCuenta_Vigencias] (
    [cnsctvo_vgnca_estdo_estdo_cnta] [dbo].[udtConsecutivo] NULL,
    [cnsctvo_cdgo_estdo_estdo_cnta]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_estdo_estdo_cnta]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_estdo_estdo_cnta]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_tbEstadosEstadosCuenta] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_estdo_estdo_cnta] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbEstadosEstadosCuenta_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbEstadosEstadosCuenta_Vigencias_tbEstadosEstadosCuenta] FOREIGN KEY ([cnsctvo_cdgo_estdo_estdo_cnta]) REFERENCES [dbo].[tbEstadosEstadosCuenta] ([cnsctvo_cdgo_estdo_estdo_cnta]) ON DELETE CASCADE
);
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbEstadosEstadosCuenta_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbEstadosEstadosCuenta_Vigencias] ON   [dbo].[tbEstadosEstadosCuenta_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_estdo_estdo_cnta	,
		@cdgo_ara = cdgo_estdo_estdo_cnta		,
		@cdgo_vgnca	= cnsctvo_vgnca_estdo_estdo_cnta,
		@dscrpcn =  dscrpcn_estdo_estdo_cnta
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbEstadosEstadosCuenta_Vigencias p
	...
	And	p.cnsctvo_cdgo_estdo_estdo_cnta = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_estdo_estdo_cnta =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_estdo_estdo_cnta !=  @cdgo_vgnca	
...
update tbEstadosEstadosCuenta set dscrpcn_estdo_estdo_cnta =  @dscrpcn
where cnsctvo_cdgo_estdo_estdo_cnta = @Codigo_Consecutivo
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbEstadosEstadosCuenta_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbEstadosEstadosCuenta_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbEstadosEstadosCuenta_Vigencias] ON   [dbo].[tbEstadosEstadosCuenta_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbEstadosEstadosCuenta_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_estdo_estdo_cnta
from INSERTED
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_estdo_estdo_cnta = n.cnsctvo_vgnca_estdo_estdo_cnta)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbEstadosEstadosCuenta_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbEstadosEstadosCuenta y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbEstadosEstadosCuenta_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbEstadosEstadosCuenta_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbIncidenciasCarteraPac 		D\>
* 			  <\D Cuando se actualiza un Campo de tbEstadosEstadosCuenta_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbEstadosEstadosCuenta_Vigencias]   On [dbo].[tbEstadosEstadosCuenta_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbEstadosEstadosCuenta_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbEstadosEstadosCuenta_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_estdo_estdo_cnta, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_estdo_estdo_cnta
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_estdo_estdo_cnta = n.cnsctvo_cdgo_estdo_estdo_cnta AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_estdo_estdo_cnta, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_estdo_estdo_cnta
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_estdo_estdo_cnta = n.cnsctvo_cdgo_estdo_estdo_cnta AND 
							(
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosEstadosCuenta_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosEstadosCuenta_Vigencias_usro_crcn]    ON    [dbo].[tbEstadosEstadosCuenta_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbEstadosEstadosCuenta_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbEstadosEstadosCuenta_Vigencias] ON   [dbo].[tbEstadosEstadosCuenta_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_estdo_estdo_cnta,
	      @cnsctvo_vgnca = cnsctvo_vgnca_estdo_estdo_cnta			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_estdo_estdo_cnta = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_estdo_estdo_cnta != @cnsctvo_vgnca
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosEstadosCuenta_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosEstadosCuenta_Vigencias]
ON  [dbo].[tbEstadosEstadosCuenta_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstadosEstadosCuenta_Vigencias ','cdgo_estdo_estdo_cnta')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_estdo_estdo_cnta = @lcCodigov  Or  cdgo_estdo_estdo_cnta =  @lcCodigoi)
Begin
	RollBack Transaction
