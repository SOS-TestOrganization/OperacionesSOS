CREATE TABLE [dbo].[tbPeriodosliquidacion_Vigencias] (
    [cnsctvo_vgnca_prdo_lqdcn] [dbo].[udtConsecutivo]    NOT NULL,
    [cnsctvo_cdgo_prdo_lqdcn]  [dbo].[udtConsecutivo]    NOT NULL,
    [cdgo_prdo_lqdcn]          CHAR (4)                  NOT NULL,
    [dscrpcn_prdo_lqdcn]       [dbo].[udtDescripcion]    NOT NULL,
    ...
    [fcha_mxma_pgo]            DATETIME                  NOT NULL,
    [fcha_crte]                DATETIME                  NOT NULL,
    [fcha_pgo]                 DATETIME                  NOT NULL,
    [intrss_mra_mnsl]          [dbo].[udtValorDecimales] NOT NULL,
    [cnsctvo_cdgo_estdo_prdo]  [dbo].[udtConsecutivo]    NOT NULL,
    [cnsctvo_grpo_lqdcn]       [dbo].[udtConsecutivo]    NOT NULL,
    [fcha_incl_prdo_lqdcn]     DATETIME                  NOT NULL,
    [fcha_fnl_prdo_lqdcn]      DATETIME                  NOT NULL,
    [tme_stmp]                 ROWVERSION                NOT NULL,
    CONSTRAINT [PK_tbPeriodosliquidacion01] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_prdo_lqdcn] ASC) WITH (FILLFACTOR = 90),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbPeriodosliquidacion_Vigencias02] CHECK ([fn_vgnca]>=[inco_vgnca]),
    CONSTRAINT [FK_tbPeriodosliquidacion_Vigencias02_tbEstadosPeriodo] FOREIGN KEY ([cnsctvo_cdgo_estdo_prdo]) REFERENCES [dbo].[tbEstadosPeriodo] ([cnsctvo_cdgo_estdo_prdo]),
    CONSTRAINT [FK_tbPeriodosliquidacion_Vigencias02_TbGruposLiquidacion] FOREIGN KEY ([cnsctvo_grpo_lqdcn]) REFERENCES [dbo].[tbGruposLiquidacion] ([cnsctvo_cdgo_grpo_lqdcn]),
    CONSTRAINT [FK_tbPeriodosliquidacion_Vigencias02_tbPeriodosliquidacion] FOREIGN KEY ([cnsctvo_cdgo_prdo_lqdcn]) REFERENCES [dbo].[tbPeriodosliquidacion] ([cnsctvo_cdgo_prdo_lqdcn]) ON DELETE CASCADE
);


GO
CREATE STATISTICS [cdgo_prdo_lqdcn]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([cdgo_prdo_lqdcn]);


GO
CREATE STATISTICS [dscrpcn_prdo_lqdcn]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([dscrpcn_prdo_lqdcn]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([fcha_crcn]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([usro_crcn]);


GO
CREATE STATISTICS [obsrvcns]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([obsrvcns]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([vsble_usro]);


GO
CREATE STATISTICS [fcha_crte]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([fcha_crte]);


GO
CREATE STATISTICS [fcha_pgo]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([fcha_pgo]);


GO
CREATE STATISTICS [intrss_mra_mnsl]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([intrss_mra_mnsl]);


GO
CREATE STATISTICS [cnsctvo_grpo_lqdcn]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([cnsctvo_grpo_lqdcn]);


GO
CREATE STATISTICS [tme_stmp]
    ON [dbo].[tbPeriodosliquidacion_Vigencias]([tme_stmp]);


GO

/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbPeriodosliquidacion_Vigencias02
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbInsidenciasCarterapac 		D\>
* 			  <\D Cuando se actualiza un Campo de tbPeriodosliquidacion_Vigencias D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbPeriodosliquidacion_Vigencias02] On [dbo].[tbPeriodosliquidacion_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbPeriodosliquidacion_Vigencias'
	SET @mascara_1_64 	=0x0000000000300000  --de derecha a izquierda: 0010000000000000010000001011100001111101000010000000000000100000
	SET @mascara_128_65 	= 0x0000000000000000  --de izquierda a derecha
	SET @mascara_combinada	= 0x300000  
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'tbPeriodosliquidacion_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_prdo_lqdcn, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_prdo_lqdcn
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_prdo_lqdcn = n.cnsctvo_cdgo_prdo_lqdcn AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_prdo_lqdcn, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_prdo_lqdcn
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_prdo_lqdcn = n.cnsctvo_cdgo_prdo_lqdcn AND 
							(
...
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbPeriodosliquidacion_Vigencias02
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbPeriodosliquidacion_Vigencias02 contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbPeriodosliquidacion_Vigencias02] ON   [dbo].[tbPeriodosliquidacion_Vigencias]
FOR UPDATE
AS 
Declare   
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_prdo_lqdcn = n.cnsctvo_vgnca_prdo_lqdcn)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbPeriodosliquidacion_Vigencias02
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbPeriodosliquidacion y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbPeriodosliquidacion_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbPeriodosliquidacion_Vigencias02
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbPeriodosliquidacion_Vigencias02] ON   [dbo].[tbPeriodosliquidacion_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_prdo_lqdcn	,
		@cdgo_ara = cdgo_prdo_lqdcn		,
		@cdgo_vgnca	= cnsctvo_vgnca_prdo_lqdcn,
		@dscrpcn =  dscrpcn_prdo_lqdcn
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbPeriodosliquidacion_Vigencias p
	...
	And	p.cnsctvo_cdgo_prdo_lqdcn = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_prdo_lqdcn =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_prdo_lqdcn !=  @cdgo_vgnca	
...
update tbPeriodosliquidacion set dscrpcn_prdo_lqdcn =  @dscrpcn
where cnsctvo_cdgo_prdo_lqdcn = @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeInicioFinPeriodo_tbPeriodosliquidacion_Vigencias02
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				A\>
* Descripcion		: <\D Valida que no se traslape  la Vigencia Nueva con Alguna existente para el mismo parametro D\>
* Observaciones		: <\O		O\>
* Parametros		: <\P   P\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeInicioFinPeriodo_tbPeriodosliquidacion_Vigencias02]  ON   [dbo].[tbPeriodosliquidacion_Vigencias]
FOR  INSERT
AS
Begin

SET NOCOUNT ON
	DECLARE	@fcha_incl_prdo_lqdcn	datetime,     
			@fcha_fnl_prdo_lqdcn	datetime,      
			@tfcha_incl_prdo_lqdcn		datetime,     
			@tfcha_fnl_prdo_lqdcn		datetime,      
			@mnsje			varchar(400), 
			@contador		int,
			@Codigo_Consecutivo	udtConsecutivo,
		  	@cdgo_ara		char(4),
...
SELECT	@fcha_incl_prdo_lqdcn = fcha_incl_prdo_lqdcn,
		@fcha_fnl_prdo_lqdcn	= fcha_fnl_prdo_lqdcn,
		@Codigo_Consecutivo = cnsctvo_cdgo_prdo_lqdcn	,
		@cdgo_ara = cdgo_prdo_lqdcn		,
		@cdgo_vgnca	= cnsctvo_vgnca_prdo_lqdcn,
		@dscrpcn =  dscrpcn_prdo_lqdcn
...
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbPeriodosliquidacion_Vigencias p
	WHERE ( (convert(varchar(10),p.fcha_incl_prdo_lqdcn,111)  >= convert(varchar(10),@fcha_incl_prdo_lqdcn,111) 
	And	convert(varchar(10),p.fcha_incl_prdo_lqdcn,111)  <= convert(varchar(10),@fcha_fnl_prdo_lqdcn,111))   Or
		(convert(varchar(10),p.fcha_fnl_prdo_lqdcn,111)    >= convert(varchar(10),@fcha_incl_prdo_lqdcn,111) 
	And	 convert(varchar(10),p.fcha_fnl_prdo_lqdcn,111)    <= convert(varchar(10),@fcha_fnl_prdo_lqdcn,111))   Or
		(convert(varchar(10),p.fcha_incl_prdo_lqdcn,111)  <= convert(varchar(10),@fcha_incl_prdo_lqdcn,111) 
	And	convert(varchar(10),p.fcha_fnl_prdo_lqdcn,111)     >= convert(varchar(10),@fcha_fnl_prdo_lqdcn,111)))  
	And	(p.cnsctvo_cdgo_prdo_lqdcn <> 0 or  cnsctvo_cdgo_prdo_lqdcn <>99999)
...
-->Para Indicar al usuario contra que fecha hay traslape
		SELECT @tfcha_incl_prdo_lqdcn = fcha_incl_prdo_lqdcn,@tfcha_fnl_prdo_lqdcn = fcha_fnl_prdo_lqdcn
		FROM  tbPeriodosliquidacion_Vigencias p
		WHERE ( (convert(varchar(10),p.fcha_incl_prdo_lqdcn,111)  >= convert(varchar(10),@fcha_incl_prdo_lqdcn,111) 
		And	convert(varchar(10),p.fcha_incl_prdo_lqdcn,111)  <= convert(varchar(10),@fcha_fnl_prdo_lqdcn,111))   Or
			(convert(varchar(10),p.fcha_fnl_prdo_lqdcn,111)    >= convert(varchar(10),@fcha_incl_prdo_lqdcn,111) 
		...
		And	convert(varchar(10),p.fcha_fnl_prdo_lqdcn,111)     >= convert(varchar(10),@fcha_fnl_prdo_lqdcn,111)))  
		And	(p.cnsctvo_cdgo_prdo_lqdcn <> 0 or   cnsctvo_cdgo_prdo_lqdcn <>99999)
		And	(p.cnsctvo_vgnca_prdo_lqdcn <> 0 or  cnsctvo_vgnca_prdo_lqdcn <>99999)
		
		
		ROLLBACK TRANSACTION
		select @mnsje  =  'Error, Insertando, Existe un periodo  con el rango de fechas    '+  convert(varchar(10),@tfcha_incl_prdo_lqdcn,111) + '  ' + convert(varchar(10),@tfcha_fnl_prdo_lqdcn,111)
		RAISERROR (999905, 16, 1,  @mnsje)


		Delete From  tbPeriodosliquidacion	Where	 cnsctvo_cdgo_prdo_lqdcn	= @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbPeriodosliquidacion_Vigencias02
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbPeriodosliquidacion_Vigencias02_usro_crcn]    ON    [dbo].[tbPeriodosliquidacion_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbPeriodosliquidacion_Vigencias02
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbPeriodosliquidacion_Vigencias02]
ON  [dbo].[tbPeriodosliquidacion_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbPeriodosliquidacion_Vigencias ','cdgo_prdo_lqdcn')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_prdo_lqdcn = @lcCodigov  Or  cdgo_prdo_lqdcn =  @lcCodigoi)
Begin
	RollBack Transaction
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbPeriodosliquidacion_Vigencias02
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbPeriodosliquidacion_Vigencias02] ON   [dbo].[tbPeriodosliquidacion_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_prdo_lqdcn,
	      @cnsctvo_vgnca = cnsctvo_vgnca_prdo_lqdcn			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_prdo_lqdcn = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_prdo_lqdcn != @cnsctvo_vgnca
