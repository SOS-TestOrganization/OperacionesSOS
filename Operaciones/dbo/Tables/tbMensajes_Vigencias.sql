CREATE TABLE [dbo].[tbMensajes_Vigencias] (
    [cnsctvo_vgnca_mnsje] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_mnsje]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_mnsje]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_mnsje]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_TbMensajes] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_mnsje] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbMensajes_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbMensajes_Vigencias_tbMensajes] FOREIGN KEY ([cnsctvo_cdgo_mnsje]) REFERENCES [dbo].[tbMensajes] ([cnsctvo_cdgo_mnsje]) ON DELETE CASCADE
);


GO
CREATE STATISTICS [cdgo_mnsje]
    ON [dbo].[tbMensajes_Vigencias]([cdgo_mnsje], [cnsctvo_vgnca_mnsje]);


GO
CREATE STATISTICS [dscrpcn_mnsje]
    ON [dbo].[tbMensajes_Vigencias]([dscrpcn_mnsje], [cnsctvo_vgnca_mnsje]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbMensajes_Vigencias]([fcha_crcn], [cnsctvo_vgnca_mnsje]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbMensajes_Vigencias]([usro_crcn], [cnsctvo_vgnca_mnsje]);


GO
CREATE STATISTICS [obsrvcns]
    ON [dbo].[tbMensajes_Vigencias]([obsrvcns], [cnsctvo_vgnca_mnsje]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbMensajes_Vigencias]([vsble_usro], [cnsctvo_vgnca_mnsje]);


GO

/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbMensajes_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbMensajes_Vigencias] ON   [dbo].[tbMensajes_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_mnsje	,
		@cdgo_ara = cdgo_mnsje		,
		@cdgo_vgnca	= cnsctvo_vgnca_mnsje,
		@dscrpcn =  dscrpcn_mnsje
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbMensajes_Vigencias p
	...
	And	p.cnsctvo_cdgo_mnsje = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_mnsje =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_mnsje !=  @cdgo_vgnca	
...
update tbMensajes set dscrpcn_mnsje =  @dscrpcn
where cnsctvo_cdgo_mnsje = @Codigo_Consecutivo
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbMensajes_Vigencias
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbMensajes_Vigencias]
ON  [dbo].[tbMensajes_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbMensajes_Vigencias ','cdgo_mnsje')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_mnsje = @lcCodigov  Or  cdgo_mnsje =  @lcCodigoi)
Begin
	RollBack Transaction
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbMensajes_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbMensajes_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbMensajes_Vigencias] ON   [dbo].[tbMensajes_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbMensajes_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_mnsje
from INSERTED
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_mnsje = n.cnsctvo_vgnca_mnsje)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbMensajes_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbMensajes y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbMensajes_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbMensajes_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrIncidencias_tbMensajes_Vigencias]  On [dbo].[tbMensajes_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbMensajes_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'tbMensajes_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_mnsje, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_mnsje
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_mnsje = n.cnsctvo_cdgo_mnsje AND 
							(
							 (
					...
					d.cnsctvo_vgnca_mnsje, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_mnsje
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_mnsje = n.cnsctvo_cdgo_mnsje AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbMensajes_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbMensajes_Vigencias] ON   [dbo].[tbMensajes_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_mnsje,
	      @cnsctvo_vgnca = cnsctvo_vgnca_mnsje			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_mnsje = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_mnsje != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbMensajes_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbMensajes_Vigencias_usro_crcn]    ON    [dbo].[tbMensajes_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
