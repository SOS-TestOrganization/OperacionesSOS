CREATE TABLE [dbo].[tbGruposxConcepto_Vigencias] (
    [cnsctvo_vgnca_grpo_cncpto] [dbo].[udtConsecutivo]    NOT NULL,
    [cnsctvo_grpo_cncpto]       [dbo].[udtConsecutivo]    NOT NULL,
    [inco_vgnca]                DATETIME                  NOT NULL,
    [fn_vgnca]                  DATETIME                  NOT NULL,
    [fcha_crcn]                 DATETIME                  NOT NULL,
    [usro_crcn]                 [dbo].[udtUsuario]        NOT NULL,
    [cnsctvo_cdgo_grpo_lqdcn]   [dbo].[udtConsecutivo]    NOT NULL,
    [cnsctvo_cdgo_cncpto_lqdcn] [dbo].[udtConsecutivo]    NOT NULL,
    [prcntje]                   [dbo].[udtValorDecimales] NOT NULL,
    [tme_stmp]                  ROWVERSION                NOT NULL,
    CONSTRAINT [PK_TbGruposxConcepto01] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_grpo_cncpto] ASC) WITH (FILLFACTOR = 90),
    CONSTRAINT [CK_tbGruposxConcepto_Vigencias02] CHECK ([inco_vgnca]<[fn_vgnca]),
    CONSTRAINT [FK_tbGruposxConcepto_Vigencias02_tbGruposxConcepto] FOREIGN KEY ([cnsctvo_grpo_cncpto]) REFERENCES [dbo].[tbGruposxConcepto] ([cnsctvo_grpo_cncpto]) ON DELETE CASCADE
);


GO
CREATE STATISTICS [cnsctvo_grpo_cncpto]
    ON [dbo].[tbGruposxConcepto_Vigencias]([cnsctvo_grpo_cncpto]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbGruposxConcepto_Vigencias]([fcha_crcn]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbGruposxConcepto_Vigencias]([usro_crcn]);


GO
CREATE STATISTICS [cnsctvo_cdgo_grpo_lqdcn]
    ON [dbo].[tbGruposxConcepto_Vigencias]([cnsctvo_cdgo_grpo_lqdcn]);


GO
CREATE STATISTICS [prcntje]
    ON [dbo].[tbGruposxConcepto_Vigencias]([prcntje]);


GO
CREATE STATISTICS [tme_stmp]
    ON [dbo].[tbGruposxConcepto_Vigencias]([tme_stmp]);
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbGruposxConcepto_Vigencias02
...
* 			  <\D Cuando se actualiza un Campo de tbGruposxConcepto_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbGruposxConcepto_Vigencias02]  On [dbo].[tbGruposxConcepto_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbGruposxConcepto_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 3
						...
						)SELECT 'bdCarteraPac', 'tbGruposxConcepto_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_grpo_cncpto, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_grpo_cncpto
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_grpo_cncpto = n.cnsctvo_grpo_cncpto AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 4
						...
						d.cnsctvo_vgnca_grpo_cncpto, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_grpo_cncpto
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_grpo_cncpto = n.cnsctvo_grpo_cncpto AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Metodo o PRG 	            	 :   TrValidaTraslapeRel_tbGruposxConcepto_Vigencias02
...
CREATE TRIGGER   [dbo].[TrValidaTraslapeRel_tbGruposxConcepto_Vigencias02]  ON    [dbo].[tbGruposxConcepto_Vigencias]
FOR  INSERT , UPDATE
AS
declare @Identificador_Tabla1		udtconsecutivo,
	@Identificador_Tabla2		udtconsecutivo,
...
SELECT    @inco_vgnca_Rel = inco_vgnca,
	    ...
	    @Identificador_Tabla1= cnsctvo_cdgo_grpo_lqdcn, 
	    @Identificador_Tabla2  = cnsctvo_cdgo_cncpto_lqdcn,
	    @cnsctvo_vgnca = cnsctvo_vgnca_grpo_cncpto
FROM        INSERTED
...
SELECT @contador=count(*)
	FROM  tbGruposLiquidacion_Vigencias p
	...
	And	p.cnsctvo_cdgo_grpo_lqdcn = @Identificador_Tabla1
...
If    @contador<1     
	 Begin
	      ...
	      select @mnsje  =  'Error, no cumple con el rango de fechas en el Parámetro tbGruposLiquidacion_Vigencias'  
...
set @contador=0
SELECT @contador=count(*)
	FROM tbConceptosLiquidacion_Vigencias02 p
	WHERE convert(varchar(10),p.inco_vgnca,111)  <= convert(varchar(10),@inco_vgnca_Rel,111)  and 
		convert(varchar(10),p.fn_vgnca,111)    >= convert(varchar(10),@fn_vgnca_Rel,111) 
	And	p.cnsctvo_cdgo_cncpto_lqdcn  =  @Identificador_Tabla2
...
If    @contador<1      
	 Begin
	      ROLLBACK TRANSACTION
	      select @mnsje  =  'Error, no cumple con el rango de fechas en el Parámetro tbConceptosLiquidacion_Vigencias02  '   + ltrim(rtrim(str(@Identificador_Tabla2))) + ' ' + convert(varchar(10),@inco_vgnca_Rel,111) + '  ' + convert(varchar(10),@fn_vgnca_Rel,111)
	      RAISERROR (999901, 16, 1,  @mnsje)
	      RETURN
	 End
...
--Se Calcula si hay  traslape de fechas
 SELECT  @contador   =  count(*)
 FROM	tbGruposxConcepto_Vigencias02
 ...
 And           cnsctvo_cdgo_grpo_lqdcn   = @Identificador_Tabla1
 And           cnsctvo_cdgo_cncpto_lqdcn = @Identificador_Tabla2

IF @contador > 1
          --Hay traslape de fechas
	 Begin
		SELECT @tinco_vgnca=inco_vgnca,@tfn_vgnca=fn_vgnca
		FROM      tbGruposxConcepto_Vigencias02 p
		WHERE ( ( convert(varchar(10),inco_vgnca,111)  >=  convert(varchar(10),@inco_vgnca_Rel,111) 
 		And             convert(varchar(10),inco_vgnca,111) <=   convert(varchar(10),@fn_vgnca_Rel,111))   Or
			      (convert(varchar(10),fn_vgnca,111) 	>=   convert(varchar(10),@inco_vgnca_Rel,111) 
		 And             convert(varchar(10),fn_vgnca,111)	<=   convert(varchar(10),@fn_vgnca_Rel,111))   Or
		 ...
		 And           cnsctvo_cdgo_grpo_lqdcn = @Identificador_Tabla1
		 And           cnsctvo_cdgo_cncpto_lqdcn = @Identificador_Tabla2
  	      	and       p.cnsctvo_vgnca_grpo_cncpto != @cnsctvo_vgnca

		
		ROLLBACK TRANSACTION
		--select @mnsje  =  'Error, Insertando, Existe  Traslape con  '+  convert(varchar(10),@tinco_vgnca,111) + '  ' + convert(varchar(10),@tfn_vgnca,111) convert(varchar(10),@inco_vgnca_Rel,111) + '  ' + convert(varchar(10),@fn_vgnca_Rel,111)
		select @mnsje  =  'Error, Insertando, Existe  Traslape con  '+  ltrim(rtrim(str(@contador)))  + ' ' +   ltrim(rtrim(str(@Identificador_Tabla1))) + ' '  +  ltrim(rtrim(str(@Identificador_Tabla2)))   + convert(varchar(10),@inco_vgnca_Rel,111) + '  ' + convert(varchar(10),@fn_vgnca_Rel,111)
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeRel_tbGruposxConcepto_Vigencias02
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeRel_tbGruposxConcepto_Vigencias02]    ON   [dbo].[tbGruposxConcepto_Vigencias]
FOR  UPDATE
AS
Begin
...
SET NOCOUNT ON
  ...
  --Se capturan las variables que se estan tratando de insertar
	SELECT	@inco_vgnca = inco_vgnca,
			@fn_vgnca	= fn_vgnca,
			@Codigo_Consecutivo = cnsctvo_grpo_cncpto
	FROM   INSERTED
...
--    Se capturan las variables que se estan modificando
	SELECT     @dinco_vgnca   =  inco_vgnca,
	        		      @dfn_vgnca     =  fn_vgnca,
		     	      @dcnsctvo_tbla = cnsctvo_grpo_cncpto,
			@cnsctvo_vgnca       = cnsctvo_vgnca_grpo_cncpto     
	FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_grpo_cncpto = n.cnsctvo_grpo_cncpto)
...
GO
/*--------------------------------------------------------------------------------------------------------
* Nombre                 	:	TrReferencia_Usuario_tbGruposxConcepto_Vigencias02
...
* Fecha Modificacion	: <\FM		FM\>
*------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_Usuario_tbGruposxConcepto_Vigencias02]  ON    [dbo].[tbGruposxConcepto_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
