CREATE TABLE [dbo].[tbBasesAplicadas_Vigencias] (
    [cnsctvo_vgnca_bse_aplcda] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_bse_aplcda]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_tpo_bse_aplcda]      [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_bse_aplcda]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_TbBasesAplicadas] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_bse_aplcda] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbBasesAplicadas_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbBasesAplicadas_Vigencias_tbBasesAplicadas] FOREIGN KEY ([cnsctvo_cdgo_bse_aplcda]) REFERENCES [dbo].[tbBasesAplicadas] ([cnsctvo_cdgo_bse_aplcda])
);


GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbBasesAplicadas_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbBasesAplicadas_Vigencias] ON   [dbo].[tbBasesAplicadas_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_bse_aplcda	,
		@cdgo_ara = cdgo_tpo_bse_aplcda		,
		@cdgo_vgnca	= cnsctvo_vgnca_bse_aplcda,
		@dscrpcn =  dscrpcn_bse_aplcda
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbBasesAplicadas_Vigencias p
	...
	And	p.cnsctvo_cdgo_bse_aplcda = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_bse_aplcda =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_bse_aplcda !=  @cdgo_vgnca	
...
update tbBasesAplicadas set dscrpcn_bse_aplcda =  @dscrpcn
where cnsctvo_cdgo_bse_aplcda = @Codigo_Consecutivo
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbBasesAplicadas_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbBasesAplicadas_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbBasesAplicadas_Vigencias] ON   [dbo].[tbBasesAplicadas_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbBasesAplicadas_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_bse_aplcda
from INSERTED
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_bse_aplcda = n.cnsctvo_vgnca_bse_aplcda)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbBasesAplicadas_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbBasesAplicadas y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbBasesAplicadas_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbBasesAplicadas_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbIncidenciasCarteraPac 		D\>
* 			  <\D Cuando se actualiza un Campo de tbBasesAplicadas_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbBasesAplicadas_Vigencias]   On [dbo].[tbBasesAplicadas_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbBasesAplicadas_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'tbBasesAplicadas_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_bse_aplcda, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_bse_aplcda
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_bse_aplcda = n.cnsctvo_cdgo_bse_aplcda AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_bse_aplcda, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_bse_aplcda
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_bse_aplcda = n.cnsctvo_cdgo_bse_aplcda AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbBasesAplicadas_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbBasesAplicadas_Vigencias] ON   [dbo].[tbBasesAplicadas_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_bse_aplcda,
	      @cnsctvo_vgnca = cnsctvo_vgnca_bse_aplcda			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_bse_aplcda = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_bse_aplcda != @cnsctvo_vgnca
...
GO
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbBasesAplicadas_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbBasesAplicadas_Vigencias_usro_crcn]    ON    [dbo].[tbBasesAplicadas_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbBasesAplicadas_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbBasesAplicadas_Vigencias]
ON  [dbo].[tbBasesAplicadas_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbBasesAplicadas_Vigencias ','cdgo_tpo_bse_aplcda')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_tpo_bse_aplcda = @lcCodigov  Or  cdgo_tpo_bse_aplcda =  @lcCodigoi)
Begin
	RollBack Transaction
