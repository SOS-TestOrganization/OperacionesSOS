CREATE TABLE [dbo].[tbTiposAplicacionNota] (
    ...
    [vsble_usro]                  [dbo].[udtCodigo]      NOT NULL,
    CONSTRAINT [PK_tbTiposAplicacionNota_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_tpo_aplccn_nta] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbTiposAplicacionNota
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbTiposAplicacionNota \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbTiposAplicacionNota] ON    [dbo].[tbTiposAplicacionNota]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbTiposAplicacionNota'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_tpo_aplccn_nta,
	@cdgo = cdgo_tpo_aplccn_nta,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_tpo_aplccn_nta from tbTiposAplicacionNota
	where  cnsctvo_cdgo_tpo_aplccn_nta = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbTiposAplicacionNota', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbTiposAplicacionNota_vigencias where  cnsctvo_cdgo_tpo_aplccn_nta = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbTiposAplicacionNota where  cnsctvo_cdgo_tpo_aplccn_nta = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposAplicacionNota
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposAplicacionNota]
ON  [dbo].[tbTiposAplicacionNota]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposAplicacionNota','cdgo_tpo_aplccn_nta')) - 1
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposAplicacionNota
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposAplicacionNota_usro_crcn]    ON    [dbo].[tbTiposAplicacionNota]
FOR  INSERT,UPDATE
AS
Begin
