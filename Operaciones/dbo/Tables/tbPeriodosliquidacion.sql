CREATE TABLE [dbo].[tbPeriodosliquidacion] (
    ...
    CONSTRAINT [PK_tbPeriodosliquidacion_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_prdo_lqdcn] ASC) WITH (FILLFACTOR = 90),
    CONSTRAINT [IX_tbPeriodosliquidacion] UNIQUE NONCLUSTERED ([cdgo_prdo_lqdcn] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbPeriodosliquidacion
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbPeriodosliquidacion \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbPeriodosliquidacion] ON    [dbo].[tbPeriodosliquidacion]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbPeriodosliquidacion'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_prdo_lqdcn,
	@cdgo = cdgo_prdo_lqdcn,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_prdo_lqdcn from tbPeriodosliquidacion
	where  cnsctvo_cdgo_prdo_lqdcn = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbPeriodosliquidacion', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbPeriodosliquidacion_vigencias where  cnsctvo_cdgo_prdo_lqdcn = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbPeriodosliquidacion where  cnsctvo_cdgo_prdo_lqdcn = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbPeriodosliquidacion
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbPeriodosliquidacion]
ON  [dbo].[tbPeriodosliquidacion]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbPeriodosliquidacion','cdgo_prdo_lqdcn')) - 1
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbPeriodosliquidacion
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbPeriodosliquidacion_usro_crcn]    ON    [dbo].[tbPeriodosliquidacion]
FOR  INSERT,UPDATE
AS
Begin
