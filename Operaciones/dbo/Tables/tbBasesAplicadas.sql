CREATE TABLE [dbo].[tbBasesAplicadas] (
    ...
    CONSTRAINT [PK_TbBasesAplicadas_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_bse_aplcda] ASC),
    CONSTRAINT [IX_tbBasesAplicadas] UNIQUE NONCLUSTERED ([cdgo_tpo_bse_aplcda] ASC)
);


GO
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbBasesAplicadas
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbBasesAplicadas \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbBasesAplicadas] ON    [dbo].[tbBasesAplicadas]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbBasesAplicadas'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_bse_aplcda,
	@cdgo = cdgo_tpo_bse_aplcda,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_bse_aplcda from tbBasesAplicadas
	where  cnsctvo_cdgo_bse_aplcda = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbBasesAplicadas', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbBasesAplicadas_vigencias where  cnsctvo_cdgo_bse_aplcda = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbBasesAplicadas where  cnsctvo_cdgo_bse_aplcda = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbBasesAplicadas
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbBasesAplicadas_usro_crcn]    ON    [dbo].[tbBasesAplicadas]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbBasesAplicadas
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbBasesAplicadas]
ON  [dbo].[tbBasesAplicadas]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbBasesAplicadas','cdgo_tpo_bse_aplcda')) - 1
