CREATE TABLE [dbo].[tbTiposPromotor] (
    ...
    CONSTRAINT [PK_TbTiposPromotor_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_tpo_prmtr] ASC)
);


GO
CREATE STATISTICS [cdgo_tpo_prmtr]
    ON [dbo].[tbTiposPromotor]([cdgo_tpo_prmtr], [cnsctvo_cdgo_tpo_prmtr]);


GO
CREATE STATISTICS [dscrpcn_tpo_prmtr]
    ON [dbo].[tbTiposPromotor]([dscrpcn_tpo_prmtr], [cnsctvo_cdgo_tpo_prmtr]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbTiposPromotor]([fcha_crcn], [cnsctvo_cdgo_tpo_prmtr]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbTiposPromotor]([usro_crcn], [cnsctvo_cdgo_tpo_prmtr]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbTiposPromotor]([vsble_usro], [cnsctvo_cdgo_tpo_prmtr]);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbTiposPromotor
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbTiposPromotor \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbTiposPromotor] ON    [dbo].[tbTiposPromotor]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbTiposPromotor'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_tpo_prmtr,
	@cdgo = cdgo_tpo_prmtr,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_tpo_prmtr from tbTiposPromotor
	where  cnsctvo_cdgo_tpo_prmtr = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbTiposPromotor', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbTiposPromotor_vigencias where  cnsctvo_cdgo_tpo_prmtr = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbTiposPromotor where  cnsctvo_cdgo_tpo_prmtr = @cnsctvo_tbla
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposPromotor
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposPromotor_usro_crcn]    ON    [dbo].[tbTiposPromotor]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposPromotor
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposPromotor]
ON  [dbo].[tbTiposPromotor]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposPromotor','cdgo_tpo_prmtr')) - 1
