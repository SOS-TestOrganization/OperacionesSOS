CREATE TABLE [dbo].[tbTiposMovimiento_Vigencias] (
    [cnsctvo_vgnca_tpo_mvmnto] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_tpo_mvmnto]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_tpo_mvmnto]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_tpo_mvmnto]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_tbTiposMovimiento] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_tpo_mvmnto] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbTiposMovimiento_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbTiposMovimiento_Vigencias_tbTiposMovimiento] FOREIGN KEY ([cnsctvo_cdgo_tpo_mvmnto]) REFERENCES [dbo].[tbTiposMovimiento] ([cnsctvo_cdgo_tpo_mvmnto]) ON DELETE CASCADE
);


GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbTiposMovimiento_Vigencias
...
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbTiposMovimiento_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbTiposMovimiento_Vigencias] ON   [dbo].[tbTiposMovimiento_Vigencias]
FOR UPDATE
AS 
Declare   
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_tpo_mvmnto = n.cnsctvo_vgnca_tpo_mvmnto)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbTiposMovimiento_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbTiposMovimiento y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbTiposMovimiento_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbTiposMovimiento_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbTiposMovimiento_Vigencias] ON   [dbo].[tbTiposMovimiento_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_tpo_mvmnto	,
		@cdgo_ara = cdgo_tpo_mvmnto		,
		@cdgo_vgnca	= cnsctvo_vgnca_tpo_mvmnto,
		@dscrpcn =  dscrpcn_tpo_mvmnto
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbTiposMovimiento_Vigencias p
	...
	And	p.cnsctvo_cdgo_tpo_mvmnto = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_mvmnto =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_tpo_mvmnto !=  @cdgo_vgnca	
...
update tbTiposMovimiento set dscrpcn_tpo_mvmnto =  @dscrpcn
where cnsctvo_cdgo_tpo_mvmnto = @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbTiposMovimiento_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbModosAplicacion_Vigencias 		D\>
* 			  <\D Cuando se actualiza un Campo de tbTiposMovimiento_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbTiposMovimiento_Vigencias]   On [dbo].[tbTiposMovimiento_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbTiposMovimiento_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'tbTiposMovimiento_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_tpo_mvmnto, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_mvmnto
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_mvmnto = n.cnsctvo_cdgo_tpo_mvmnto AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_tpo_mvmnto, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_mvmnto
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_mvmnto = n.cnsctvo_cdgo_tpo_mvmnto AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbTiposMovimiento_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbTiposMovimiento_Vigencias] ON   [dbo].[tbTiposMovimiento_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_tpo_mvmnto,
	      @cnsctvo_vgnca = cnsctvo_vgnca_tpo_mvmnto			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_mvmnto = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_tpo_mvmnto != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposMovimiento_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposMovimiento_Vigencias_usro_crcn]    ON    [dbo].[tbTiposMovimiento_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposMovimiento_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposMovimiento_Vigencias]
ON  [dbo].[tbTiposMovimiento_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposMovimiento_Vigencias ','cdgo_tpo_mvmnto')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_tpo_mvmnto = @lcCodigov  Or  cdgo_tpo_mvmnto =  @lcCodigoi)
Begin
	RollBack Transaction
