CREATE TABLE [dbo].[tbTiposConsecutivo] (
    ...
    CONSTRAINT [PK_tbTiposConsecutivo_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_tpo_cnsctvo] ASC)
);


GO
CREATE STATISTICS [cdgo_tpo_cnsctvo]
    ON [dbo].[tbTiposConsecutivo]([cdgo_tpo_cnsctvo], [cnsctvo_cdgo_tpo_cnsctvo]);


GO
CREATE STATISTICS [dscrpcn_tpo_cnsctvo]
    ON [dbo].[tbTiposConsecutivo]([dscrpcn_tpo_cnsctvo], [cnsctvo_cdgo_tpo_cnsctvo]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbTiposConsecutivo]([fcha_crcn], [cnsctvo_cdgo_tpo_cnsctvo]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbTiposConsecutivo]([usro_crcn], [cnsctvo_cdgo_tpo_cnsctvo]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbTiposConsecutivo]([vsble_usro], [cnsctvo_cdgo_tpo_cnsctvo]);


GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposConsecutivo
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposConsecutivo]
ON  [dbo].[tbTiposConsecutivo]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposConsecutivo','cdgo_tpo_cnsctvo')) - 1
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbTiposConsecutivo
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbTiposConsecutivo \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbTiposConsecutivo] ON    [dbo].[tbTiposConsecutivo]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbTiposConsecutivo'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_tpo_cnsctvo,
	@cdgo = cdgo_tpo_cnsctvo,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_tpo_cnsctvo from tbTiposConsecutivo
	where  cnsctvo_cdgo_tpo_cnsctvo = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbTiposConsecutivo', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbTiposConsecutivo_vigencias where  cnsctvo_cdgo_tpo_cnsctvo = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbTiposConsecutivo where  cnsctvo_cdgo_tpo_cnsctvo = @cnsctvo_tbla
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposConsecutivo
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposConsecutivo_usro_crcn]    ON    [dbo].[tbTiposConsecutivo]
FOR  INSERT,UPDATE
AS
Begin
