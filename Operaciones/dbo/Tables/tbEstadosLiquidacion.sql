CREATE TABLE [dbo].[tbEstadosLiquidacion] (
    [cnsctvo_cdgo_estdo_lqdcn] [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_estdo_lqdcn]         [dbo].[udtCodigo]      NULL,
    [dscrpcn_estdo_lqdcn]      [dbo].[udtDescripcion] NULL,
    [fcha_crcn]                DATETIME               NULL,
    [usro_crcn]                [dbo].[udtUsuario]     NULL,
    [vsble_usro]               [dbo].[udtLogico]      NULL,
    CONSTRAINT [PK_tbEstadosLiquidacion_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_estdo_lqdcn] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbEstadosLiquidacion
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbEstadosLiquidacion \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbEstadosLiquidacion] ON    [dbo].[tbEstadosLiquidacion]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbEstadosLiquidacion'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_estdo_lqdcn,
	@cdgo = cdgo_estdo_lqdcn,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_estdo_lqdcn from tbEstadosLiquidacion
	where  cnsctvo_cdgo_estdo_lqdcn = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbEstadosLiquidacion', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbEstadosLiquidacion_vigencias where  cnsctvo_cdgo_estdo_lqdcn = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbEstadosLiquidacion where  cnsctvo_cdgo_estdo_lqdcn = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosLiquidacion
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosLiquidacion]
ON  [dbo].[tbEstadosLiquidacion]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstadosLiquidacion','cdgo_estdo_lqdcn')) - 1
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosLiquidacion
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosLiquidacion_usro_crcn]    ON    [dbo].[tbEstadosLiquidacion]
FOR  INSERT,UPDATE
AS
Begin
