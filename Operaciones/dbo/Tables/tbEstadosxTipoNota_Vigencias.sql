CREATE TABLE [dbo].[tbEstadosxTipoNota_Vigencias] (
    [cnsctvo_vgnca_tpo_nta]  [dbo].[udtConsecutivo] NOT NULL,
    ...
    CONSTRAINT [PK_TbEstadosxTipoNota] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_tpo_nta] ASC),
    CONSTRAINT [CK_tbEstadosxTipoNota_Vigencias] CHECK ([inco_vgnca] < [fn_vgnca]),
    CONSTRAINT [FK_tbEstadosxTipoNota_Vigencias_tbEstadosNota] FOREIGN KEY ([cnsctvo_cdgo_estdo_nta]) REFERENCES [dbo].[tbEstadosNota] ([cnsctvo_cdgo_estdo_nta]),
    CONSTRAINT [FK_tbEstadosxTipoNota_Vigencias_tbEstadosxTipoNota] FOREIGN KEY ([cnsctvo_estdo_tpo_nta]) REFERENCES [dbo].[tbEstadosxTipoNota] ([cnsctvo_estdo_tpo_nta]) ON DELETE CASCADE,
    CONSTRAINT [FK_tbEstadosxTipoNota_Vigencias_tbTiposNotas] FOREIGN KEY ([cnsctvo_cdgo_tpo_nta]) REFERENCES [dbo].[tbTiposNotas] ([cnsctvo_cdgo_tpo_nta])
);


GO
CREATE STATISTICS [cnsctvo_estdo_tpo_nta]
    ON [dbo].[tbEstadosxTipoNota_Vigencias]([cnsctvo_estdo_tpo_nta], [cnsctvo_vgnca_tpo_nta]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbEstadosxTipoNota_Vigencias]([fcha_crcn], [cnsctvo_vgnca_tpo_nta]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbEstadosxTipoNota_Vigencias]([usro_crcn], [cnsctvo_vgnca_tpo_nta]);


GO
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosxTipoNota_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosxTipoNota_Vigencias_usro_crcn]    ON    [dbo].[tbEstadosxTipoNota_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbEstadosxTipoNota_Vigencias
...
* 			  <\D Cuando se actualiza un Campo de  TbEstadosxTipoNota_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbEstadosxTipoNota_Vigencias]   On [dbo].[tbEstadosxTipoNota_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbEstadosxTipoNota_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 3
						...
						)SELECT 'bdCarteraPac', 'TbEstadosxTipoNota_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_tpo_nta, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_estdo_tpo_nta
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_estdo_tpo_nta = n.cnsctvo_estdo_tpo_nta AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 4
						...
						d.cnsctvo_vgnca_tpo_nta, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_estdo_tpo_nta
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_estdo_tpo_nta = n.cnsctvo_estdo_tpo_nta AND 
							(
...
GO
/*--------------------------------------------------------------------------------------------------------
* Nombre                 	:	TrReferencia_Usuario_tbEstadosxTipoNota_Vigencias
* Desarrollado por	: <\A	Ing. Jorge Meneses			  A\>
* Descripcion	 	: <\D	Garantiza integridad referencial con una tabla en otra base de Datos D\>
* Observaciones     	: <\O	O\>
* Parametros		: <\P	P\>
...
* Fecha Modificacion	: <\FM		FM\>
*------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_Usuario_tbEstadosxTipoNota_Vigencias] ON    [dbo].[tbEstadosxTipoNota_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
	Declare 	@lcmnsje	varchar(400),
...
GO
/*---------------------------------------------------------------------------------
* Metodo o PRG 	            	 :   TrValidaTraslapeRel_tbEstadosxTipoNota_Vigencias
...
CREATE TRIGGER   [dbo].[TrValidaTraslapeRel_tbEstadosxTipoNota_Vigencias]  ON    [dbo].[tbEstadosxTipoNota_Vigencias]
FOR  INSERT,UPDATE
AS
...
SELECT    @inco_vgnca_Rel = inco_vgnca,
	    ...
	    @Identificador_Tabla1= cnsctvo_cdgo_tpo_nta, 
	    @Identificador_Tabla2  = cnsctvo_cdgo_estdo_nta,
	    @cnsctvo_vgnca = cnsctvo_vgnca_tpo_nta
FROM        INSERTED
...
SELECT @contador=count(*)
	...
	And	p.cnsctvo_cdgo_tpo_nta = @Identificador_Tabla1
...
If    @contador<1     
	 Begin
	      ROLLBACK TRANSACTION
	      select @mnsje  =  'Error, no cumple con el rango de fechas en el Parámetro tbTiposNotas_Vigencias'  
...
set @contador=0
SELECT @contador=count(*)
	FROM tbEstadosNota_Vigencias p
	WHERE convert(varchar(10),p.inco_vgnca,111)  <= convert(varchar(10),@inco_vgnca_Rel,111)  and 
		convert(varchar(10),p.fn_vgnca,111)    >= convert(varchar(10),@fn_vgnca_Rel,111) 
	And	p.cnsctvo_cdgo_estdo_nta  =  @Identificador_Tabla2
...
If    @contador<1      
	 Begin
	      ROLLBACK TRANSACTION
	      select @mnsje  =  'Error, no cumple con el rango de fechas en el Parámetro tbEstadosNota_Vigencias '  
	      RAISERROR (999901, 16, 1,  @mnsje)
	      RETURN
	 End
...
--Se Calcula si hay  traslape de fechas
 SELECT  @contador   =  count(*)
 FROM	tbEstadosxTipoNota_Vigencias
 ...
 And           cnsctvo_cdgo_tpo_nta = @Identificador_Tabla1
 And           cnsctvo_cdgo_estdo_nta = @Identificador_Tabla2

IF @contador > 1
          --Hay traslape de fechas
	 Begin
		SELECT @tinco_vgnca=inco_vgnca,@tfn_vgnca=fn_vgnca
		FROM tbEstadosxTipoNota_Vigencias p
		WHERE ( ( convert(varchar(10),inco_vgnca,111)  >=  convert(varchar(10),@inco_vgnca_Rel,111) 
 		And             convert(varchar(10),inco_vgnca,111) <=   convert(varchar(10),@fn_vgnca_Rel,111))   Or
			      (convert(varchar(10),fn_vgnca,111) 	>=   convert(varchar(10),@inco_vgnca_Rel,111) 
		 And             convert(varchar(10),fn_vgnca,111)	<=   convert(varchar(10),@fn_vgnca_Rel,111))   Or
		 ...
		 And           cnsctvo_cdgo_tpo_nta = @Identificador_Tabla1
		 And           cnsctvo_cdgo_estdo_nta = @Identificador_Tabla2
  	      	and       p.cnsctvo_vgnca_tpo_nta != @cnsctvo_vgnca
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeRel_tbEstadosxTipoNota_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeRel_tbEstadosxTipoNota_Vigencias]    ON   [dbo].[tbEstadosxTipoNota_Vigencias]
FOR  UPDATE
AS
Begin
...
SET NOCOUNT ON
  ...
  --Se capturan las variables que se estan tratando de insertar
	SELECT	@inco_vgnca = inco_vgnca,
			@fn_vgnca	= fn_vgnca,
			@Codigo_Consecutivo = cnsctvo_estdo_tpo_nta
	FROM   INSERTED
...
--    Se capturan las variables que se estan modificando
	SELECT     @dinco_vgnca   =  inco_vgnca,
	        		      @dfn_vgnca     =  fn_vgnca,
		     	      @dcnsctvo_tbla = cnsctvo_estdo_tpo_nta,
			@cnsctvo_vgnca = cnsctvo_vgnca_tpo_nta     
	FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_estdo_tpo_nta = n.cnsctvo_estdo_tpo_nta)
...
--Valida que el fin de vigencia no sea inferior a fecha actual  para un parametro vigente
   ...
   /*     --Se Calcula si hay  traslape de fechas
	...
	And	p.cnsctvo_estdo_tpo_nta = @Codigo_Consecutivo
