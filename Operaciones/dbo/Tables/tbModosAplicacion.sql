CREATE TABLE [dbo].[tbModosAplicacion] (
    ...
    CONSTRAINT [PK_tbModosAplicacion_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_mdo_aplccn] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbModosAplicacion
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbModosAplicacion \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbModosAplicacion] ON    [dbo].[tbModosAplicacion]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbModosAplicacion'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_mdo_aplccn,
	@cdgo = cdgo_mdo_aplccn,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_mdo_aplccn from tbModosAplicacion
	where  cnsctvo_cdgo_mdo_aplccn = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbModosAplicacion', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbModosAplicacion_vigencias where  cnsctvo_cdgo_mdo_aplccn = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbModosAplicacion where  cnsctvo_cdgo_mdo_aplccn = @cnsctvo_tbla
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbModosAplicacion
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbModosAplicacion_usro_crcn]    ON    [dbo].[tbModosAplicacion]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbModosAplicacion
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbModosAplicacion]
ON  [dbo].[tbModosAplicacion]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbModosAplicacion','cdgo_mdo_aplccn')) - 1
