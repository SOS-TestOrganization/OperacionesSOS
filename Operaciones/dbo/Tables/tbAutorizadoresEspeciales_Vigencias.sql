CREATE TABLE [dbo].[tbAutorizadoresEspeciales_Vigencias] (
    [cnsctvo_vgnca_autrzdr_espcl] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_autrzdr_espcl]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_autrzdr_espcl]          CHAR (10)              NOT NULL,
    [dscrpcn_autrzdr_espcl]       [dbo].[udtDescripcion] NOT NULL,
    ...
    [prmr_nmbre_usro]             CHAR (30)              NULL,
    [sgndo_nmbre_usro]            CHAR (30)              NULL,
    [prmr_aplldo_usro]            CHAR (30)              NULL,
    [sgndo_aplldo_usro]           CHAR (30)              NULL,
    [tme_stmp]                    ROWVERSION             NOT NULL,
    CONSTRAINT [PK_tbAutorizadoresEspeciales] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_autrzdr_espcl] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbAutorizadoresEspeciales_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbAutorizadoresEspeciales_Vigencias_tbAutorizadoresEspeciales] FOREIGN KEY ([cnsctvo_cdgo_autrzdr_espcl]) REFERENCES [dbo].[tbAutorizadoresEspeciales] ([cnsctvo_cdgo_autrzdr_espcl]) ON DELETE CASCADE
);
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbAutorizadoresEspeciales_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbAutorizadoresEspeciales_Vigencias] ON   [dbo].[tbAutorizadoresEspeciales_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_autrzdr_espcl	,
		@cdgo_ara = cdgo_autrzdr_espcl		,
		@cdgo_vgnca	= cnsctvo_vgnca_autrzdr_espcl,
		@dscrpcn =  dscrpcn_autrzdr_espcl
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbAutorizadoresEspeciales_Vigencias p
	...
	And	p.cnsctvo_cdgo_autrzdr_espcl = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_autrzdr_espcl =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_autrzdr_espcl !=  @cdgo_vgnca	
...
update tbAutorizadoresEspeciales set dscrpcn_autrzdr_espcl =  @dscrpcn
where cnsctvo_cdgo_autrzdr_espcl = @Codigo_Consecutivo
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbAutorizadoresEspeciales_Vigencias
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbAutorizadoresEspeciales_Vigencias]
ON  [dbo].[tbAutorizadoresEspeciales_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbAutorizadoresEspeciales_Vigencias ','cdgo_autrzdr_espcl')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_autrzdr_espcl = @lcCodigov  Or  cdgo_autrzdr_espcl =  @lcCodigoi)
Begin
	RollBack Transaction
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbAutorizadoresEspeciales_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbAutorizadoresEspeciales_Vigencias] ON   [dbo].[tbAutorizadoresEspeciales_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_autrzdr_espcl,
	      @cnsctvo_vgnca = cnsctvo_vgnca_autrzdr_espcl			     
FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_autrzdr_espcl = n.cnsctvo_vgnca_autrzdr_espcl )
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_autrzdr_espcl = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_autrzdr_espcl != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbAutorizadoresEspeciales_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbAutorizadoresEspeciales_Vigencias_usro_crcn]    ON    [dbo].[tbAutorizadoresEspeciales_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
																																																		TRIGGERS NO PRINCIPALES
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/



/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbAutorizadoresEspeciales_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbIncidenciasCarteraPac 		D\>
* 			  <\D Cuando se actualiza un Campo de tbAutorizadoresEspeciales_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbAutorizadoresEspeciales_Vigencias]   On [dbo].[tbAutorizadoresEspeciales_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbAutorizadoresEspeciales_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'tbAutorizadoresEspeciales_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_autrzdr_espcl, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_autrzdr_espcl
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_autrzdr_espcl = n.cnsctvo_cdgo_autrzdr_espcl AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_autrzdr_espcl, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_autrzdr_espcl
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_autrzdr_espcl = n.cnsctvo_cdgo_autrzdr_espcl AND 
							(
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbAutorizadoresEspeciales_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbAutorizadoresEspeciales_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbAutorizadoresEspeciales_Vigencias] ON   [dbo].[tbAutorizadoresEspeciales_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbAutorizadoresEspeciales_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_autrzdr_espcl
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbAutorizadoresEspeciales_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbAutorizadoresEspeciales y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbAutorizadoresEspeciales_Vigencias  y  ' + @lcopcion_menu	
