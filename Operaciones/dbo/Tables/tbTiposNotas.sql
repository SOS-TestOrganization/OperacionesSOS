CREATE TABLE [dbo].[tbTiposNotas] (
    ...
    CONSTRAINT [PK_tbTiposNotas_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_tpo_nta] ASC)
);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbTiposNotas]([fcha_crcn], [cnsctvo_cdgo_tpo_nta]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbTiposNotas]([usro_crcn], [cnsctvo_cdgo_tpo_nta]);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbTiposNotas
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbTiposNotas \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbTiposNotas] ON    [dbo].[tbTiposNotas]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbTiposNotas'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_tpo_nta,
	@cdgo = cdgo_tpo_nta,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_tpo_nta from tbTiposNotas
	where  cnsctvo_cdgo_tpo_nta = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbTiposNotas', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbTiposNotas_vigencias where  cnsctvo_cdgo_tpo_nta = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbTiposNotas where  cnsctvo_cdgo_tpo_nta = @cnsctvo_tbla
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposNotas
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposNotas_usro_crcn]    ON    [dbo].[tbTiposNotas]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposNotas
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposNotas]
ON  [dbo].[tbTiposNotas]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposNotas','cdgo_tpo_nta')) - 1
