CREATE TABLE [dbo].[tbEstadosNota] (
    ...
    CONSTRAINT [PK_tbEstadosNota_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_estdo_nta] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbEstadosNota
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbEstadosNota \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbEstadosNota] ON    [dbo].[tbEstadosNota]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbEstadosNota'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_estdo_nta,
	@cdgo = cdgo_estdo_nta,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_estdo_nta from tbEstadosNota
	where  cnsctvo_cdgo_estdo_nta = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbEstadosNota', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbEstadosNota_vigencias where  cnsctvo_cdgo_estdo_nta = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbEstadosNota where  cnsctvo_cdgo_estdo_nta = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosNota
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosNota]
ON  [dbo].[tbEstadosNota]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstadosNota','cdgo_estdo_nta')) - 1
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosNota
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosNota_usro_crcn]    ON    [dbo].[tbEstadosNota]
FOR  INSERT,UPDATE
AS
Begin
