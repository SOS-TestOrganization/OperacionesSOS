CREATE TABLE [dbo].[tbEstadosPeriodo] (
    ...
    CONSTRAINT [PK_tbEstadosPeriodo_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_estdo_prdo] ASC)
);
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosPeriodo
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosPeriodo_usro_crcn]    ON    [dbo].[tbEstadosPeriodo]
FOR  INSERT,UPDATE
AS
Begin
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbEstadosPeriodo
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbEstadosPeriodo \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbEstadosPeriodo] ON    [dbo].[tbEstadosPeriodo]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbEstadosPeriodo'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_estdo_prdo,
	@cdgo = cdgo_estdo_prdo,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_estdo_prdo from tbEstadosPeriodo
	where  cnsctvo_cdgo_estdo_prdo = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbEstadosPeriodo', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbEstadosPeriodo_vigencias where  cnsctvo_cdgo_estdo_prdo = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbEstadosPeriodo where  cnsctvo_cdgo_estdo_prdo = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosPeriodo
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosPeriodo]
ON  [dbo].[tbEstadosPeriodo]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstadosPeriodo','cdgo_estdo_prdo')) - 1
