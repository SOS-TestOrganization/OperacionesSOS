CREATE TABLE [dbo].[tbEstadosLiquidacion_Vigencias] (
    [cnsctvo_vgnca_estdo_lqdcn] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_estdo_lqdcn]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_estdo_lqdcn]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_estdo_lqdcn]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_tbEstadosLiquidacion] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_estdo_lqdcn] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbEstadosLiquidacion_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbEstadosLiquidacion_Vigencias_tbEstadosLiquidacion] FOREIGN KEY ([cnsctvo_cdgo_estdo_lqdcn]) REFERENCES [dbo].[tbEstadosLiquidacion] ([cnsctvo_cdgo_estdo_lqdcn]) ON DELETE CASCADE
);
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbEstadosLiquidacion_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbEstadosLiquidacion_Vigencias] ON   [dbo].[tbEstadosLiquidacion_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_estdo_lqdcn	,
		@cdgo_ara = cdgo_estdo_lqdcn		,
		@cdgo_vgnca	= cnsctvo_vgnca_estdo_lqdcn,
		@dscrpcn =  dscrpcn_estdo_lqdcn
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbEstadosLiquidacion_Vigencias p
	...
	And	p.cnsctvo_cdgo_estdo_lqdcn = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_estdo_lqdcn =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_estdo_lqdcn !=  @cdgo_vgnca	
...
update tbEstadosLiquidacion set dscrpcn_estdo_lqdcn =  @dscrpcn
where cnsctvo_cdgo_estdo_lqdcn = @Codigo_Consecutivo
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbEstadosLiquidacion_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbEstadosLiquidacion_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbEstadosLiquidacion_Vigencias] ON   [dbo].[tbEstadosLiquidacion_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbEstadosLiquidacion_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_estdo_lqdcn
from INSERTED
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_estdo_lqdcn = n.cnsctvo_vgnca_estdo_lqdcn)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbEstadosLiquidacion_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbEstadosLiquidacion y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbEstadosLiquidacion_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbEstadosLiquidacion_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbIncidenciasCarteraPac 		D\>
* 			  <\D Cuando se actualiza un Campo de tbEstadosLiquidacion_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbEstadosLiquidacion_Vigencias]   On  [dbo].[tbEstadosLiquidacion_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbEstadosLiquidacion_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbEstadosLiquidacion_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_estdo_lqdcn, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_estdo_lqdcn
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_estdo_lqdcn = n.cnsctvo_cdgo_estdo_lqdcn AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_estdo_lqdcn, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_estdo_lqdcn
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_estdo_lqdcn = n.cnsctvo_cdgo_estdo_lqdcn AND 
							(
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosLiquidacion_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosLiquidacion_Vigencias_usro_crcn]    ON    [dbo].[tbEstadosLiquidacion_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbEstadosLiquidacion_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbEstadosLiquidacion_Vigencias] ON   [dbo].[tbEstadosLiquidacion_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_estdo_lqdcn,
	      @cnsctvo_vgnca = cnsctvo_vgnca_estdo_lqdcn			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_estdo_lqdcn = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_estdo_lqdcn != @cnsctvo_vgnca
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosLiquidacion_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosLiquidacion_Vigencias]
ON  [dbo].[tbEstadosLiquidacion_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstadosLiquidacion_Vigencias ','cdgo_estdo_lqdcn')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_estdo_lqdcn = @lcCodigov  Or  cdgo_estdo_lqdcn =  @lcCodigoi)
Begin
	RollBack Transaction
