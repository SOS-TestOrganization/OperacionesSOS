CREATE TABLE [dbo].[tbTiposMorosidad_Vigencias] (
    [cnsctvo_vgnca_tpo_mrsdd] [dbo].[udtConsecutivo]  NOT NULL,
    [cnsctvo_cdgo_tpo_mrsdd]  [dbo].[udtConsecutivo]  NOT NULL,
    [cdgo_tpo_mrsdd]          [dbo].[udtCodigo]       NOT NULL,
    [dscrpcn_tpo_mrsdd]       [dbo].[udtDescripcion]  NOT NULL,
    ...
    [ds_mrsdd]                [dbo].[udtValorPequeno] NOT NULL,
    [tme_stmp]                ROWVERSION              NOT NULL,
    CONSTRAINT [PK_tbTiposMorosidad] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_tpo_mrsdd] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbTiposMorosidad_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbTiposMorosidad_Vigencias_tbTiposMorosidad] FOREIGN KEY ([cnsctvo_cdgo_tpo_mrsdd]) REFERENCES [dbo].[tbTiposMorosidad] ([cnsctvo_cdgo_tpo_mrsdd]) ON DELETE CASCADE
);


GO
CREATE STATISTICS [cdgo_tpo_mrsdd]
    ON [dbo].[tbTiposMorosidad_Vigencias]([cdgo_tpo_mrsdd], [cnsctvo_vgnca_tpo_mrsdd]);


GO
CREATE STATISTICS [dscrpcn_tpo_mrsdd]
    ON [dbo].[tbTiposMorosidad_Vigencias]([dscrpcn_tpo_mrsdd], [cnsctvo_vgnca_tpo_mrsdd]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbTiposMorosidad_Vigencias]([fcha_crcn], [cnsctvo_vgnca_tpo_mrsdd]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbTiposMorosidad_Vigencias]([usro_crcn], [cnsctvo_vgnca_tpo_mrsdd]);


GO
CREATE STATISTICS [obsrvcns]
    ON [dbo].[tbTiposMorosidad_Vigencias]([obsrvcns], [cnsctvo_vgnca_tpo_mrsdd]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbTiposMorosidad_Vigencias]([vsble_usro], [cnsctvo_vgnca_tpo_mrsdd]);


GO
CREATE STATISTICS [ds_mrsdd]
    ON [dbo].[tbTiposMorosidad_Vigencias]([ds_mrsdd], [cnsctvo_vgnca_tpo_mrsdd]);


GO
CREATE STATISTICS [tme_stmp]
    ON [dbo].[tbTiposMorosidad_Vigencias]([tme_stmp]);


GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbTiposMorosidad_Vigencias
...
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbTiposMorosidad_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbTiposMorosidad_Vigencias] ON   [dbo].[tbTiposMorosidad_Vigencias]
FOR UPDATE
AS 
Declare   
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_tpo_mrsdd = n.cnsctvo_vgnca_tpo_mrsdd)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbTiposMorosidad_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbTiposMorosidad y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbTiposMorosidad_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbTiposMorosidad_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbTiposMorosidad_Vigencias] ON   [dbo].[tbTiposMorosidad_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_tpo_mrsdd	,
		@cdgo_ara = cdgo_tpo_mrsdd		,
		@cdgo_vgnca	= cnsctvo_vgnca_tpo_mrsdd,
		@dscrpcn =  dscrpcn_tpo_mrsdd
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbTiposMorosidad_Vigencias p
	...
	And	p.cnsctvo_cdgo_tpo_mrsdd = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_mrsdd =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_tpo_mrsdd !=  @cdgo_vgnca	
...
update tbTiposMorosidad set dscrpcn_tpo_mrsdd =  @dscrpcn
where cnsctvo_cdgo_tpo_mrsdd = @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposMorosidad_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposMorosidad_Vigencias_usro_crcn]    ON    [dbo].[tbTiposMorosidad_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbTiposMorosidad_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbTiposMorosidad_Vigencias] ON   [dbo].[tbTiposMorosidad_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_tpo_mrsdd,
	      @cnsctvo_vgnca = cnsctvo_vgnca_tpo_mrsdd			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_mrsdd = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_tpo_mrsdd != @cnsctvo_vgnca
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposMorosidad_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposMorosidad_Vigencias]
ON  [dbo].[tbTiposMorosidad_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposMorosidad_Vigencias ','cdgo_tpo_mrsdd')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_tpo_mrsdd = @lcCodigov  Or  cdgo_tpo_mrsdd =  @lcCodigoi)
Begin
	RollBack Transaction
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbTiposMorosidad_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbModosAplicacion_Vigencias 		D\>
* 			  <\D Cuando se actualiza un Campo de TbTiposMorosidad_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbTiposMorosidad_Vigencias]   On [dbo].[tbTiposMorosidad_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbTiposMorosidad_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbTiposMorosidad_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_tpo_mrsdd, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_mrsdd
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_mrsdd = n.cnsctvo_cdgo_tpo_mrsdd AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_tpo_mrsdd, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_mrsdd
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_mrsdd = n.cnsctvo_cdgo_tpo_mrsdd AND 
							(
