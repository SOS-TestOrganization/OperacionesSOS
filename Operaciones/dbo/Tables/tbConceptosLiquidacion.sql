CREATE TABLE [dbo].[tbConceptosLiquidacion] (
    ...
    CONSTRAINT [PK_TbConceptosliquidacion_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_cncpto_lqdcn] ASC) WITH (FILLFACTOR = 90)
);


GO
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbConceptosliquidacion
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbConceptosliquidacion_usro_crcn]    ON    [dbo].[tbConceptosLiquidacion]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbConceptosliquidacion
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbConceptosliquidacion]
ON  [dbo].[tbConceptosLiquidacion]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbConceptosliquidacion','cdgo_cncpto_lqdcn')) - 1
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbConceptosliquidacion
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbConceptosliquidacion \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbConceptosliquidacion] ON    [dbo].[tbConceptosLiquidacion]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbConceptosliquidacion'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_cncpto_lqdcn,
	@cdgo = cdgo_cncpto_lqdcn,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_cncpto_lqdcn from tbConceptosliquidacion
	where  cnsctvo_cdgo_cncpto_lqdcn = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbConceptosliquidacion', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbConceptosliquidacion_vigencias where  cnsctvo_cdgo_cncpto_lqdcn = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbConceptosliquidacion where  cnsctvo_cdgo_cncpto_lqdcn = @cnsctvo_tbla
