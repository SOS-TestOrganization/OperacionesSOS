CREATE TABLE [dbo].[tbTiposAplicacionNota_Vigencias] (
    [cnsctvo_vgnca_tpo_aplccn_nta] CHAR (10)              NOT NULL,
    [cnsctvo_cdgo_tpo_aplccn_nta]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_tpo_aplccn_nta]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_tpo_aplccn_nta]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_tbTiposAplicacionNota] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_tpo_aplccn_nta] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbTiposAplicacionNota_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbTiposAplicacionNota_Vigencias_tbTiposAplicacionNota] FOREIGN KEY ([cnsctvo_cdgo_tpo_aplccn_nta]) REFERENCES [dbo].[tbTiposAplicacionNota] ([cnsctvo_cdgo_tpo_aplccn_nta]) ON DELETE CASCADE
);
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbTiposAplicacionNota_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbTiposAplicacionNota_Vigencias] ON   [dbo].[tbTiposAplicacionNota_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_tpo_aplccn_nta	,
		@cdgo_ara = cdgo_tpo_aplccn_nta		,
		@cdgo_vgnca	= cnsctvo_vgnca_tpo_aplccn_nta,
		@dscrpcn =  dscrpcn_tpo_aplccn_nta
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbTiposAplicacionNota_Vigencias p
	...
	And	p.cnsctvo_cdgo_tpo_aplccn_nta = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_aplccn_nta =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_tpo_aplccn_nta !=  @cdgo_vgnca	
...
update tbTiposAplicacionNota set dscrpcn_tpo_aplccn_nta =  @dscrpcn
where cnsctvo_cdgo_tpo_aplccn_nta = @Codigo_Consecutivo
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbTiposAplicacionNota_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbTiposAplicacionNota_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbTiposAplicacionNota_Vigencias] ON   [dbo].[tbTiposAplicacionNota_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbTiposAplicacionNota_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_tpo_aplccn_nta
from INSERTED
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_tpo_aplccn_nta = n.cnsctvo_vgnca_tpo_aplccn_nta)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbTiposAplicacionNota_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbTiposAplicacionNota y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbTiposAplicacionNota_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbTiposAplicacionNota_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbModosAplicacion_Vigencias 		D\>
* 			  <\D Cuando se actualiza un Campo de tbTiposAplicacionNota_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbTiposAplicacionNota_Vigencias]   On [dbo].[tbTiposAplicacionNota_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbTiposAplicacionNota_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbTiposAplicacionNota_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_tpo_aplccn_nta, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_aplccn_nta
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_aplccn_nta = n.cnsctvo_cdgo_tpo_aplccn_nta AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_tpo_aplccn_nta, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_aplccn_nta
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_aplccn_nta = n.cnsctvo_cdgo_tpo_aplccn_nta AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbTiposAplicacionNota_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbTiposAplicacionNota_Vigencias] ON   [dbo].[tbTiposAplicacionNota_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_tpo_aplccn_nta,
	      @cnsctvo_vgnca = cnsctvo_vgnca_tpo_aplccn_nta			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_aplccn_nta = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_tpo_aplccn_nta != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposAplicacionNota_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposAplicacionNota_Vigencias_usro_crcn]    ON    [dbo].[tbTiposAplicacionNota_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposAplicacionNota_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposAplicacionNota_Vigencias]
ON  [dbo].[tbTiposAplicacionNota_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposAplicacionNota_Vigencias ','cdgo_tpo_aplccn_nta')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_tpo_aplccn_nta = @lcCodigov  Or  cdgo_tpo_aplccn_nta =  @lcCodigoi)
Begin
	RollBack Transaction
