CREATE TABLE [dbo].[tbTiposPromotor_Vigencias] (
    [cnsctvo_vgnca_tpo_prmtr] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_tpo_prmtr]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_tpo_prmtr]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_tpo_prmtr]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_TbTiposPromotor] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_tpo_prmtr] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbTiposPromotor_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbTiposPromotor_Vigencias_tbTiposPromotor] FOREIGN KEY ([cnsctvo_cdgo_tpo_prmtr]) REFERENCES [dbo].[tbTiposPromotor] ([cnsctvo_cdgo_tpo_prmtr]) ON DELETE CASCADE
);


GO
CREATE STATISTICS [cnsctvo_cdgo_tpo_prmtr]
    ON [dbo].[tbTiposPromotor_Vigencias]([cnsctvo_cdgo_tpo_prmtr], [cnsctvo_vgnca_tpo_prmtr]);


GO
CREATE STATISTICS [cdgo_tpo_prmtr]
    ON [dbo].[tbTiposPromotor_Vigencias]([cdgo_tpo_prmtr], [cnsctvo_vgnca_tpo_prmtr]);


GO
CREATE STATISTICS [dscrpcn_tpo_prmtr]
    ON [dbo].[tbTiposPromotor_Vigencias]([dscrpcn_tpo_prmtr], [cnsctvo_vgnca_tpo_prmtr]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbTiposPromotor_Vigencias]([fcha_crcn], [cnsctvo_vgnca_tpo_prmtr]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbTiposPromotor_Vigencias]([usro_crcn], [cnsctvo_vgnca_tpo_prmtr]);


GO
CREATE STATISTICS [obsrvcns]
    ON [dbo].[tbTiposPromotor_Vigencias]([obsrvcns], [cnsctvo_vgnca_tpo_prmtr]);


GO
CREATE STATISTICS [tme_stmp]
    ON [dbo].[tbTiposPromotor_Vigencias]([tme_stmp]);


GO

/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbTiposPromotor_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbTiposPromotor_Vigencias] ON   [dbo].[tbTiposPromotor_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_tpo_prmtr	,
		@cdgo_ara = cdgo_tpo_prmtr		,
		@cdgo_vgnca	= cnsctvo_vgnca_tpo_prmtr,
		@dscrpcn =  dscrpcn_tpo_prmtr
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbTiposPromotor_Vigencias p
	...
	And	p.cnsctvo_cdgo_tpo_prmtr = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_prmtr =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_tpo_prmtr !=  @cdgo_vgnca	
...
update tbTiposPromotor set dscrpcn_tpo_prmtr =  @dscrpcn
where cnsctvo_cdgo_tpo_prmtr = @Codigo_Consecutivo
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposPromotor_Vigencias
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposPromotor_Vigencias]
ON  [dbo].[tbTiposPromotor_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposPromotor_Vigencias ','cdgo_tpo_prmtr')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_tpo_prmtr = @lcCodigov  Or  cdgo_tpo_prmtr =  @lcCodigoi)
Begin
	RollBack Transaction
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposPromotor_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposPromotor_Vigencias_usro_crcn]    ON    [dbo].[tbTiposPromotor_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbTiposPromotor_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbIncidenciasCarteraPac 		D\>
* 			  <\D Cuando se actualiza un Campo de  TbTiposPromotor_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbTiposPromotor_Vigencias]   On [dbo].[tbTiposPromotor_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbTiposPromotor_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbTiposPromotor_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_tpo_prmtr, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_prmtr
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_prmtr = n.cnsctvo_cdgo_tpo_prmtr AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_tpo_prmtr, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_prmtr
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_prmtr = n.cnsctvo_cdgo_tpo_prmtr AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbTiposPromotor_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbTiposPromotor_Vigencias] ON   [dbo].[tbTiposPromotor_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_tpo_prmtr,
	      @cnsctvo_vgnca = cnsctvo_vgnca_tpo_prmtr			     
FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_tpo_prmtr = n.cnsctvo_vgnca_tpo_prmtr )
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_prmtr = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_tpo_prmtr != @cnsctvo_vgnca
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbTiposPromotor_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbTiposPromotor_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbTiposPromotor_Vigencias] ON   [dbo].[tbTiposPromotor_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbTiposPromotor_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_tpo_prmtr
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbTiposPromotor_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbTiposPromotor y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbTiposPromotor_Vigencias  y  ' + @lcopcion_menu	
