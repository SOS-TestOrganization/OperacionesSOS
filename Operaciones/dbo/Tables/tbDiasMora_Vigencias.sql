CREATE TABLE [dbo].[tbDiasMora_Vigencias] (
    [cnsctvo_vgnca_ds_mra] [dbo].[udtConsecutivo]  NOT NULL,
    [cnsctvo_cdgo_ds_mra]  [dbo].[udtConsecutivo]  NOT NULL,
    [cdgo_ds_mra]          [dbo].[udtCodigo]       NOT NULL,
    [dscrpcn_da_mra]       [dbo].[udtDescripcion]  NOT NULL,
    ...
    [nmro_ds]              [dbo].[udtValorPequeno] NOT NULL,
    [tme_stmp]             ROWVERSION              NOT NULL,
    CONSTRAINT [PK_TbDiasMora] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_ds_mra] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbDiasMora_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca])
);


GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbDiasMora_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbDiasMora_Vigencias] ON   [dbo].[tbDiasMora_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo =  cnsctvo_cdgo_ds_mra	,
		@cdgo_ara =  cdgo_ds_mra		
FROM   INSERTED

-->    Se capturan las variables que se estan modificando
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_ds_mra,
	      @cnsctvo_vgnca = cnsctvo_vgnca_ds_mra			     
FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_ds_mra = n.cnsctvo_vgnca_ds_mra )
...
-->Se Calcula si hay  traslape de fechas
	...
	And	p.cnsctvo_cdgo_ds_mra = @Codigo_Consecutivo
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_ds_mra = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_ds_mra != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbDiasMora_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbDiasMora_Vigencias_usro_crcn]    ON    [dbo].[tbDiasMora_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbDiasMora_Vigencias
...
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbDiasMora_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbDiasMora_Vigencias] ON   [dbo].[tbDiasMora_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbDiasMora_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_ds_mra
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbDiasMora_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbDiasMora y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbDiasMora_Vigencias  y  ' + @lcopcion_menu	
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbDiasMora_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbDiasMora_Vigencias]
ON  [dbo].[tbDiasMora_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbDiasMora_Vigencias ','cdgo_ds_mra')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_ds_mra = @lcCodigov  Or  cdgo_ds_mra =  @lcCodigoi)
Begin
	RollBack Transaction
...
GO
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbDiasMora_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbIncidenciasCarteraPac 		D\>
* 			  <\D Cuando se actualiza un Campo de tbDiasMora_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbDiasMora_Vigencias]   On [dbo].[tbDiasMora_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbDiasMora_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 6
						...
						)SELECT 'bdCarteraPac', 'TbDiasMora_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_ds_mra, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_ds_mra
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_ds_mra = n.cnsctvo_cdgo_ds_mra AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 7
						...
						d.cnsctvo_vgnca_ds_mra, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_ds_mra
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_ds_mra = n.cnsctvo_cdgo_ds_mra AND 
							(
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbDiasMora_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbDiasMora_Vigencias] ON   [dbo].[tbDiasMora_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		...
		@Codigo_Consecutivo = cnsctvo_cdgo_ds_mra	,
		@cdgo_ara = cdgo_ds_mra		,
		@cdgo_vgnca	= cnsctvo_vgnca_ds_mra,
		@dscrpcn =  dscrpcn_da_mra
FROM   INSERTED
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_ds_mra =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_ds_mra !=  @cdgo_vgnca	
...
update tbDiasMora set dscrpcn_da_mra =  @dscrpcn
where cnsctvo_cdgo_ds_mra = @Codigo_Consecutivo
