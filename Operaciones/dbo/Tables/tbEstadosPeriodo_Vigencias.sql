CREATE TABLE [dbo].[tbEstadosPeriodo_Vigencias] (
    [cnsctvo_vgnca_estdo_prdo] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_estdo_prdo]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_estdo_prdo]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_estdo_prdo]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_tbEstadosPeriodo] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_estdo_prdo] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbEstadosPeriodo_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbEstadosPeriodo_Vigencias_tbEstadosPeriodo] FOREIGN KEY ([cnsctvo_cdgo_estdo_prdo]) REFERENCES [dbo].[tbEstadosPeriodo] ([cnsctvo_cdgo_estdo_prdo]) ON DELETE CASCADE
);


GO
CREATE STATISTICS [cdgo_estdo_prdo]
    ON [dbo].[tbEstadosPeriodo_Vigencias]([cdgo_estdo_prdo], [cnsctvo_vgnca_estdo_prdo]);


GO
CREATE STATISTICS [dscrpcn_estdo_prdo]
    ON [dbo].[tbEstadosPeriodo_Vigencias]([dscrpcn_estdo_prdo], [cnsctvo_vgnca_estdo_prdo]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbEstadosPeriodo_Vigencias]([fcha_crcn], [cnsctvo_vgnca_estdo_prdo]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbEstadosPeriodo_Vigencias]([usro_crcn], [cnsctvo_vgnca_estdo_prdo]);


GO
CREATE STATISTICS [obsrvcns]
    ON [dbo].[tbEstadosPeriodo_Vigencias]([obsrvcns], [cnsctvo_vgnca_estdo_prdo]);


GO
CREATE STATISTICS [tme_stmp]
    ON [dbo].[tbEstadosPeriodo_Vigencias]([tme_stmp]);


GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosPeriodo_Vigencias
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosPeriodo_Vigencias]
ON  [dbo].[tbEstadosPeriodo_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstadosPeriodo_Vigencias ','cdgo_estdo_prdo')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_estdo_prdo = @lcCodigov  Or  cdgo_estdo_prdo =  @lcCodigoi)
Begin
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbEstadosPeriodo_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbEstadosPeriodo_Vigencias] ON   [dbo].[tbEstadosPeriodo_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_estdo_prdo	,
		@cdgo_ara = cdgo_estdo_prdo		,
		@cdgo_vgnca	= cnsctvo_vgnca_estdo_prdo,
		@dscrpcn =  dscrpcn_estdo_prdo
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbEstadosPeriodo_Vigencias p
	...
	And	p.cnsctvo_cdgo_estdo_prdo = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_estdo_prdo =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_estdo_prdo !=  @cdgo_vgnca	
...
update tbEstadosPeriodo set dscrpcn_estdo_prdo =  @dscrpcn
where cnsctvo_cdgo_estdo_prdo = @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbEstadosPeriodo_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbEstadosPeriodo_Vigencias]   On [dbo].[tbEstadosPeriodo_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbEstadosPeriodo_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbEstadosPeriodo_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_estdo_prdo, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_estdo_prdo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_estdo_prdo = n.cnsctvo_cdgo_estdo_prdo AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_estdo_prdo, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_estdo_prdo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_estdo_prdo = n.cnsctvo_cdgo_estdo_prdo AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbEstadosPeriodo_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbEstadosPeriodo_Vigencias] ON   [dbo].[tbEstadosPeriodo_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_estdo_prdo,
	      @cnsctvo_vgnca = cnsctvo_vgnca_estdo_prdo			     
FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_estdo_prdo = n.cnsctvo_vgnca_estdo_prdo )
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_estdo_prdo = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_estdo_prdo != @cnsctvo_vgnca
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbEstadosPeriodo_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbEstadosPeriodo_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbEstadosPeriodo_Vigencias] ON   [dbo].[tbEstadosPeriodo_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbEstadosPeriodo_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_estdo_prdo
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbEstadosPeriodo_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbEstadosPeriodo y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbEstadosPeriodo_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosPeriodo_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosPeriodo_Vigencias_usro_crcn]    ON    [dbo].[tbEstadosPeriodo_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
