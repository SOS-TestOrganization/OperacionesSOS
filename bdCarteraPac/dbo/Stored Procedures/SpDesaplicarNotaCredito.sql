...
/*---------------------------------------------------------------------------------
* Metodo o PRG 		:  SpDesaplicarNotaCredito
...
*---------------------------------------------------------------------------------*/
CREATE  PROCEDURE SpDesaplicarNotaCredito
...
If  (@lnConsecutivoEstadoActualNota	=	5	or 	@lnConsecutivoEstadoActualNota	=	6) -- sin aplicar o anulada
	Begin 
		...
		Set	@lcMensaje		=	'No se puede desaplicar la nota su estado no lo permite'
		Return -1
	end	
...
Set	sldo_nta				=	sldo_nta  + @lntotalValorEstadosCuenta + @lnTotalValorNotasDebito ,
	cnsctvo_cdgo_estdo_nta		=	5	-- sin aplicar
From	bdcarteraPac.dbo.TbnotasPac a 
Where 	a.nmro_nta			= 	@nmro_nta
And	a.cnsctvo_cdgo_tpo_nta		=	@lcTipoNota
...
/* se comenta para  no ir a las tablas actualizar el saldo.

-- se debe de actualizar el saldo del contrato para ese responsable tanto de las notas debito como los estados de cuenta
Update	tbAcumuladoscontrato
Set	vlr_sldo		=		a.vlr_sldo	+	b.vlr	+	b.vlr_iva
From	tbAcumuladoscontrato	a,	#tmpcontratosEstadocuenta   b
