CREATE TABLE [dbo].[tbMensajes] (
    ...
    CONSTRAINT [PK_TbMensajes_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_mnsje] ASC),
    CONSTRAINT [IX_tbMensajes] UNIQUE NONCLUSTERED ([cdgo_mnsje] ASC)
);


GO
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbMensajes
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbMensajes_usro_crcn]    ON    [dbo].[tbMensajes]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbMensajes
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbMensajes \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbMensajes] ON    [dbo].[tbMensajes]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbMensajes'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_mnsje,
	@cdgo = cdgo_mnsje,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_mnsje from tbMensajes
	where  cnsctvo_cdgo_mnsje = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbMensajes', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbMensajes_vigencias where  cnsctvo_cdgo_mnsje = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbMensajes where  cnsctvo_cdgo_mnsje = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbMensajes
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbMensajes]
ON  [dbo].[tbMensajes]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbMensajes','cdgo_mnsje')) - 1
