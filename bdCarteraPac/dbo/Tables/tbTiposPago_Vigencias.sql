CREATE TABLE [dbo].[tbTiposPago_Vigencias] (
    [cnsctvo_vgnca_tpo_pgo] [dbo].[udtConsecutivo] NOT NULL,
    ...
    CONSTRAINT [PK_tbTiposPago] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_tpo_pgo] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbTiposPago_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbTiposPago_Vigencias_tbTiposPago] FOREIGN KEY ([cnsctvo_cdgo_tpo_pgo]) REFERENCES [dbo].[tbTiposPago] ([cnsctvo_cdgo_tpo_pgo]) ON DELETE CASCADE
);


GO
CREATE STATISTICS [cdgo_tpo_pgo]
    ON [dbo].[tbTiposPago_Vigencias]([cdgo_tpo_pgo], [cnsctvo_vgnca_tpo_pgo]);


GO
CREATE STATISTICS [dscrpcn_tpo_pgo]
    ON [dbo].[tbTiposPago_Vigencias]([dscrpcn_tpo_pgo], [cnsctvo_vgnca_tpo_pgo]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbTiposPago_Vigencias]([fcha_crcn], [cnsctvo_vgnca_tpo_pgo]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbTiposPago_Vigencias]([usro_crcn], [cnsctvo_vgnca_tpo_pgo]);


GO
CREATE STATISTICS [obsrvcns]
    ON [dbo].[tbTiposPago_Vigencias]([obsrvcns], [cnsctvo_vgnca_tpo_pgo]);


GO

/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbTiposPago_Vigencias
...
* 			  <\D Cuando se actualiza un Campo de  TbTiposPago_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbTiposPago_Vigencias]   On [dbo].[tbTiposPago_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbTiposPago_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbTiposPago_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_tpo_pgo, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_pgo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_pgo = n.cnsctvo_cdgo_tpo_pgo AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_tpo_pgo, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_pgo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_pgo = n.cnsctvo_cdgo_tpo_pgo AND 
							(
...
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposPago_Vigencias
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposPago_Vigencias]
ON  [dbo].[tbTiposPago_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposPago_Vigencias ','cdgo_tpo_pgo')) - 1
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbTiposPago_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbTiposPago_Vigencias] ON   [dbo].[tbTiposPago_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_tpo_pgo	,
		@cdgo_ara = cdgo_tpo_pgo		,
		@cdgo_vgnca	= cnsctvo_vgnca_tpo_pgo,
...
-->Se Calcula si hay  traslape de fechas
	...
	And	p.cnsctvo_cdgo_tpo_pgo = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_pgo =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_tpo_pgo !=  @cdgo_vgnca	
...
update tbTiposPago set dscrpcn_tpo_pgo =  @dscrpcn
where cnsctvo_cdgo_tpo_pgo = @Codigo_Consecutivo
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbTiposPago_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbTiposPago_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbTiposPago_Vigencias] ON   [dbo].[tbTiposPago_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbTiposPago_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_tpo_pgo
from INSERTED
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_tpo_pgo = n.cnsctvo_vgnca_tpo_pgo)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbTiposPago_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbTiposPago y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbTiposPago_Vigencias  y  ' + @lcopcion_menu	
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbTiposPago_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbTiposPago_Vigencias] ON   [dbo].[tbTiposPago_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_tpo_pgo,
	      @cnsctvo_vgnca = cnsctvo_vgnca_tpo_pgo			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_pgo = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_tpo_pgo != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposPago_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposPago_Vigencias_usro_crcn]    ON    [dbo].[tbTiposPago_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
