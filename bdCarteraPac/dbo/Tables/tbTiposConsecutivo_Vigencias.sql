CREATE TABLE [dbo].[tbTiposConsecutivo_Vigencias] (
    [cnsctvo_vgnca_tpo_cnsctvo] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_tpo_cnsctvo]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_tpo_cnsctvo]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_tpo_cnsctvo]       [dbo].[udtDescripcion] NOT NULL,
    ...
    [vlr_incl]                  INT                    NOT NULL,
    [vlr_antrr]                 INT                    NOT NULL,
    [vlr_actl]                  INT                    NOT NULL,
    [tme_stmp]                  ROWVERSION             NOT NULL,
    CONSTRAINT [PK_tbTiposConsecutivo] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_tpo_cnsctvo] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbTiposConsecutivo_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbTiposConsecutivo_Vigencias_tbTiposConsecutivo] FOREIGN KEY ([cnsctvo_cdgo_tpo_cnsctvo]) REFERENCES [dbo].[tbTiposConsecutivo] ([cnsctvo_cdgo_tpo_cnsctvo]) ON DELETE CASCADE
);


GO
CREATE STATISTICS [cdgo_tpo_cnsctvo]
    ON [dbo].[tbTiposConsecutivo_Vigencias]([cdgo_tpo_cnsctvo], [cnsctvo_vgnca_tpo_cnsctvo]);


GO
CREATE STATISTICS [dscrpcn_tpo_cnsctvo]
    ON [dbo].[tbTiposConsecutivo_Vigencias]([dscrpcn_tpo_cnsctvo], [cnsctvo_vgnca_tpo_cnsctvo]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbTiposConsecutivo_Vigencias]([fcha_crcn], [cnsctvo_vgnca_tpo_cnsctvo]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbTiposConsecutivo_Vigencias]([usro_crcn], [cnsctvo_vgnca_tpo_cnsctvo]);


GO
CREATE STATISTICS [obsrvcns]
    ON [dbo].[tbTiposConsecutivo_Vigencias]([obsrvcns], [cnsctvo_vgnca_tpo_cnsctvo]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbTiposConsecutivo_Vigencias]([vsble_usro], [cnsctvo_vgnca_tpo_cnsctvo]);


GO
CREATE STATISTICS [vlr_incl]
    ON [dbo].[tbTiposConsecutivo_Vigencias]([vlr_incl], [cnsctvo_vgnca_tpo_cnsctvo]);


GO
CREATE STATISTICS [vlr_antrr]
    ON [dbo].[tbTiposConsecutivo_Vigencias]([vlr_antrr], [cnsctvo_vgnca_tpo_cnsctvo]);


GO
CREATE STATISTICS [vlr_actl]
    ON [dbo].[tbTiposConsecutivo_Vigencias]([vlr_actl], [cnsctvo_vgnca_tpo_cnsctvo]);


GO

/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbTiposConsecutivo_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbTiposConsecutivo_Vigencias] ON   [dbo].[tbTiposConsecutivo_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_tpo_cnsctvo	,
		@cdgo_ara = cdgo_tpo_cnsctvo		,
		@cdgo_vgnca	= cnsctvo_vgnca_tpo_cnsctvo,
		@dscrpcn =  dscrpcn_tpo_cnsctvo
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbTiposConsecutivo_Vigencias p
	...
	And	p.cnsctvo_cdgo_tpo_cnsctvo = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_cnsctvo =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_tpo_cnsctvo !=  @cdgo_vgnca	
...
update tbTiposConsecutivo set dscrpcn_tpo_cnsctvo =  @dscrpcn
where cnsctvo_cdgo_tpo_cnsctvo = @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposConsecutivo_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposConsecutivo_Vigencias_usro_crcn]    ON    [dbo].[tbTiposConsecutivo_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbTiposConsecutivo_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbTiposConsecutivo_Vigencias] ON   [dbo].[tbTiposConsecutivo_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_tpo_cnsctvo,
	      @cnsctvo_vgnca = cnsctvo_vgnca_tpo_cnsctvo			     
FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_tpo_cnsctvo = n.cnsctvo_vgnca_tpo_cnsctvo )
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_cnsctvo = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_tpo_cnsctvo != @cnsctvo_vgnca
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbTiposConsecutivo_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbTiposConsecutivo_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbTiposConsecutivo_Vigencias] ON   [dbo].[tbTiposConsecutivo_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbTiposConsecutivo_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_tpo_cnsctvo
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbTiposConsecutivo_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbTiposConsecutivo y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbTiposConsecutivo_Vigencias  y  ' + @lcopcion_menu	
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposConsecutivo_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposConsecutivo_Vigencias]
ON  [dbo].[tbTiposConsecutivo_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposConsecutivo_Vigencias ','cdgo_tpo_cnsctvo')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_tpo_cnsctvo = @lcCodigov  Or  cdgo_tpo_cnsctvo =  @lcCodigoi)
Begin
	RollBack Transaction
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbTiposConsecutivo_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbInsidenciasCarterapac 		D\>
* 			  <\D Cuando se actualiza un Campo de tbTiposConsecutivo_Vigencias\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
*			  <\V @InsertString	 : Cadena de inserciÃ³n  			V\>
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrIncidencias_tbTiposConsecutivo_Vigencias] On [dbo].[tbTiposConsecutivo_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbTiposConsecutivo_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'tbTiposConsecutivo_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_tpo_cnsctvo, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_cnsctvo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_cnsctvo = n.cnsctvo_cdgo_tpo_cnsctvo AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_tpo_cnsctvo, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_cnsctvo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_cnsctvo = n.cnsctvo_cdgo_tpo_cnsctvo AND 
							(
