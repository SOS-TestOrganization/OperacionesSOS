CREATE TABLE [dbo].[tbDescuentos_Vigencias] (
    [cnsctvo_vgnca_dscnto]        [dbo].[udtConsecutivo] NOT NULL,
    ...
    [vlr]                         NUMERIC (13, 3)        NOT NULL,
    [espcl]                       [dbo].[udtLogico]      NOT NULL,
    [cnsctvo_cdgo_prmtro_prgrmcn] [dbo].[udtConsecutivo] NOT NULL,
    [tme_stmp]                    ROWVERSION             NOT NULL,
    CONSTRAINT [PK_TbDescuentos] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_dscnto] ASC) WITH (FILLFACTOR = 90),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbDescuentos_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbDescuentos_Vigencias_tbDescuentos] FOREIGN KEY ([cnsctvo_cdgo_dscnto]) REFERENCES [dbo].[tbDescuentos] ([cnsctvo_cdgo_dscnto]) ON DELETE CASCADE,
    CONSTRAINT [FK_tbDescuentos_Vigencias_tbParametrosProgramacionCarteraPac] FOREIGN KEY ([cnsctvo_cdgo_prmtro_prgrmcn]) REFERENCES [dbo].[tbParametrosProgramacionCarteraPac] ([cnsctvo_cdgo_prmtro_prgrmcn])
);
...
*---------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_ttbDescuentos_Vigencias]    ON [dbo].[tbDescuentos_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbDescuentos_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbDescuentos_Vigencias]
ON  [dbo].[tbDescuentos_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbDescuentos_Vigencias ','cdgo_dscnto')) - 1
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbDescuentos_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbDescuentos_Vigencias_usro_crcn]    ON    [dbo].[tbDescuentos_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbDescuentos_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbDescuentos_Vigencias] ON   [dbo].[tbDescuentos_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo =  cnsctvo_cdgo_dscnto	,
		@cdgo_ara =  cdgo_dscnto		
FROM   INSERTED

-->    Se capturan las variables que se estan modificando
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_dscnto,
	      @cnsctvo_vgnca = cnsctvo_vgnca_dscnto			     
FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_dscnto = n.cnsctvo_vgnca_dscnto )
...
-->Se Calcula si hay  traslape de fechas
	...
	And	p.cnsctvo_cdgo_dscnto = @Codigo_Consecutivo
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_dscnto = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_dscnto != @cnsctvo_vgnca
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbDescuentos_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbDescuentos_Vigencias] ON   [dbo].[tbDescuentos_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		...
		@Codigo_Consecutivo = cnsctvo_cdgo_dscnto	,
		@cdgo_ara = cdgo_dscnto		,
		@cdgo_vgnca	= cnsctvo_vgnca_dscnto,
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_dscnto =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_dscnto !=  @cdgo_vgnca	
...
update tbDescuentos set dscrpcn_dscnto =  @dscrpcn
where cnsctvo_cdgo_dscnto = @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbDescuentos_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbInsidenciasCarterapac 		D\>
* 			  <\D Cuando se actualiza un Campo de TbDescuentos_Vigencias D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbDescuentos_Vigencias]  On  [dbo].[tbDescuentos_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbDescuentos_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbDescuentos_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_dscnto, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_dscnto
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_dscnto = n.cnsctvo_cdgo_dscnto AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_dscnto, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_dscnto
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_dscnto = n.cnsctvo_cdgo_dscnto AND 
							(
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbDescuentos_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbDescuentos_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbDescuentos_Vigencias] ON   [dbo].[tbDescuentos_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbDescuentos_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_dscnto
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbDescuentos_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbDescuentos y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbDescuentos_Vigencias  y  ' + @lcopcion_menu	
