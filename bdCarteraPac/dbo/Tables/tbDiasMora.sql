CREATE TABLE [dbo].[tbDiasMora] (
    ...
    CONSTRAINT [PK_TbDiasMora_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_ds_mra] ASC)
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_tbDiasMora]
    ON [dbo].[tbDiasMora]([cdgo_ds_mra] ASC);


GO
CREATE STATISTICS [dscrpcn_da_mra]
    ON [dbo].[tbDiasMora]([dscrpcn_da_mra], [cnsctvo_cdgo_ds_mra]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbDiasMora]([fcha_crcn], [cnsctvo_cdgo_ds_mra]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbDiasMora]([usro_crcn], [cnsctvo_cdgo_ds_mra]);
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbDiasMora
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbDiasMora_usro_crcn]    ON    [dbo].[tbDiasMora]
FOR  INSERT,UPDATE
AS
Begin
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbDiasMora
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbDiasMora \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbDiasMora] ON    [dbo].[tbDiasMora]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbDiasMora'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_ds_mra,
	@cdgo = cdgo_ds_mra,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_da_mra from tbDiasMora
	where  cnsctvo_cdgo_ds_mra = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbDiasMora', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbDiasMora_vigencias where  cnsctvo_cdgo_ds_mra = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbDiasMora where  cnsctvo_cdgo_ds_mra = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbDiasMora
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbDiasMora]
ON  [dbo].[tbDiasMora]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbDiasMora','cdgo_ds_mra')) - 1
