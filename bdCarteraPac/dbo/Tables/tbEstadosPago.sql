CREATE TABLE [dbo].[tbEstadosPago] (
    [cnsctvo_cdgo_estdo_pgo] [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_estdo_pgo]         [dbo].[udtCodigo]      NULL,
    ...
    CONSTRAINT [PK_tbEstadosPago_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_estdo_pgo] ASC)
);


GO
CREATE STATISTICS [cdgo_estdo_pgo]
    ON [dbo].[tbEstadosPago]([cdgo_estdo_pgo], [cnsctvo_cdgo_estdo_pgo]);


GO
CREATE STATISTICS [dscrpcn_estdo_pgo]
    ON [dbo].[tbEstadosPago]([dscrpcn_estdo_pgo], [cnsctvo_cdgo_estdo_pgo]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbEstadosPago]([fcha_crcn], [cnsctvo_cdgo_estdo_pgo]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbEstadosPago]([usro_crcn], [cnsctvo_cdgo_estdo_pgo]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbEstadosPago]([vsble_usro], [cnsctvo_cdgo_estdo_pgo]);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbEstadosPago
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbEstadosPago \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbEstadosPago] ON    [dbo].[tbEstadosPago]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbEstadosPago'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_estdo_pgo,
	@cdgo = cdgo_estdo_pgo,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_estdo_pgo from tbEstadosPago
	where  cnsctvo_cdgo_estdo_pgo = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbEstadosPago', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbEstadosPago_vigencias where  cnsctvo_cdgo_estdo_pgo = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbEstadosPago where  cnsctvo_cdgo_estdo_pgo = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosPago
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosPago]
ON  [dbo].[tbEstadosPago]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstadosPago','cdgo_estdo_pgo')) - 1
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosPago
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosPago_usro_crcn]    ON    [dbo].[tbEstadosPago]
FOR  INSERT,UPDATE
AS
Begin
