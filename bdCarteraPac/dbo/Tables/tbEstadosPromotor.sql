CREATE TABLE [dbo].[tbEstadosPromotor] (
    ...
    CONSTRAINT [PK_TbEstadosPromotor_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_estdo_prmtr] ASC)
);


GO
CREATE STATISTICS [cdgo_estdo_prmtr]
    ON [dbo].[tbEstadosPromotor]([cdgo_estdo_prmtr], [cnsctvo_cdgo_estdo_prmtr]);


GO
CREATE STATISTICS [dscrpcn_estdo_prmtr]
    ON [dbo].[tbEstadosPromotor]([dscrpcn_estdo_prmtr], [cnsctvo_cdgo_estdo_prmtr]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbEstadosPromotor]([fcha_crcn], [cnsctvo_cdgo_estdo_prmtr]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbEstadosPromotor]([usro_crcn], [cnsctvo_cdgo_estdo_prmtr]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbEstadosPromotor]([vsble_usro], [cnsctvo_cdgo_estdo_prmtr]);
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosPromotor
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosPromotor_usro_crcn]    ON    [dbo].[tbEstadosPromotor]
FOR  INSERT,UPDATE
AS
Begin
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbEstadosPromotor
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbEstadosPromotor \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbEstadosPromotor] ON    [dbo].[tbEstadosPromotor]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbEstadosPromotor'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_estdo_prmtr,
	@cdgo = cdgo_estdo_prmtr,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_estdo_prmtr from tbEstadosPromotor
	where  cnsctvo_cdgo_estdo_prmtr = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbEstadosPromotor', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbEstadosPromotor_vigencias where  cnsctvo_cdgo_estdo_prmtr = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbEstadosPromotor where  cnsctvo_cdgo_estdo_prmtr = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosPromotor
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosPromotor]
ON  [dbo].[tbEstadosPromotor]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstadosPromotor','cdgo_estdo_prmtr')) - 1
