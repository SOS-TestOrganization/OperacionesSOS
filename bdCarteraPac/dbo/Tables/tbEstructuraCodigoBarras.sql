CREATE TABLE [dbo].[tbEstructuraCodigoBarras] (
    ...
    CONSTRAINT [PK_tbEstructuraCodigoBarras] PRIMARY KEY CLUSTERED ([cnsctvo_estrctra_cdgo_brrs] ASC)
);


GO
CREATE STATISTICS [cdgo_estrctra_cdgo_brrs]
    ON [dbo].[tbEstructuraCodigoBarras]([cdgo_estrctra_cdgo_brrs], [cnsctvo_estrctra_cdgo_brrs]);


GO
CREATE STATISTICS [dscrpcn_estrctra_cdgo_brrs]
    ON [dbo].[tbEstructuraCodigoBarras]([dscrpcn_estrctra_cdgo_brrs], [cnsctvo_estrctra_cdgo_brrs]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbEstructuraCodigoBarras]([fcha_crcn], [cnsctvo_estrctra_cdgo_brrs]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbEstructuraCodigoBarras]([usro_crcn], [cnsctvo_estrctra_cdgo_brrs]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbEstructuraCodigoBarras]([vsble_usro], [cnsctvo_estrctra_cdgo_brrs]);
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstructuraCodigoBarras
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstructuraCodigoBarras_usro_crcn]    ON    [dbo].[tbEstructuraCodigoBarras]
FOR  INSERT,UPDATE
AS
Begin
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbEstructuraCodigoBarras
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbEstructuraCodigoBarras \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbEstructuraCodigoBarras] ON    [dbo].[tbEstructuraCodigoBarras]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbEstructuraCodigoBarras'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_estrctra_cdgo_brrs,
	@cdgo = cdgo_estrctra_cdgo_brrs,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_estrctra_cdgo_brrs from tbEstructuraCodigoBarras
	where  cnsctvo_estrctra_cdgo_brrs = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbEstructuraCodigoBarras', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbEstructuraCodigoBarras_vigencias where  cnsctvo_estrctra_cdgo_brrs = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbEstructuraCodigoBarras where  cnsctvo_estrctra_cdgo_brrs = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstructuraCodigoBarras
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstructuraCodigoBarras]
ON  [dbo].[tbEstructuraCodigoBarras]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstructuraCodigoBarras','cdgo_estrctra_cdgo_brrs')) - 1
