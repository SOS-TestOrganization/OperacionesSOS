CREATE TABLE [dbo].[tbGruposLiquidacion] (
    ...
    CONSTRAINT [PK_TbGruposLiquidacion_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_grpo_lqdcn] ASC)
);


GO
CREATE STATISTICS [dscrpcn_grpo_lqdcn]
    ON [dbo].[tbGruposLiquidacion]([dscrpcn_grpo_lqdcn], [cnsctvo_cdgo_grpo_lqdcn]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbGruposLiquidacion]([fcha_crcn], [cnsctvo_cdgo_grpo_lqdcn]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbGruposLiquidacion]([usro_crcn], [cnsctvo_cdgo_grpo_lqdcn]);
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbGruposLiquidacion
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbGruposLiquidacion_usro_crcn]    ON    [dbo].[tbGruposLiquidacion]
FOR  INSERT,UPDATE
AS
Begin
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbGruposLiquidacion
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbGruposLiquidacion \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbGruposLiquidacion] ON    [dbo].[tbGruposLiquidacion]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbGruposLiquidacion'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_grpo_lqdcn,
	@cdgo = cdgo_grpo_lqdcn,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_grpo_lqdcn from tbGruposLiquidacion
	where  cnsctvo_cdgo_grpo_lqdcn = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbGruposLiquidacion', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbGruposLiquidacion_vigencias where  cnsctvo_cdgo_grpo_lqdcn = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbGruposLiquidacion where  cnsctvo_cdgo_grpo_lqdcn = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbGruposLiquidacion
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbGruposLiquidacion]
ON  [dbo].[tbGruposLiquidacion]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbGruposLiquidacion','cdgo_grpo_lqdcn')) - 1
