CREATE TABLE [dbo].[tbTipoProceso_Vigencias] (
    [cnsctvo_vgnca_tpo_prcso] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_tpo_prcso]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_tpo_prcso]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_tpo_prcso]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_tbTipoProceso_Vigencias] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_tpo_prcso] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbTipoProceso_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbTipoProceso_Vigencias_tbTipoProceso] FOREIGN KEY ([cnsctvo_cdgo_tpo_prcso]) REFERENCES [dbo].[tbTipoProceso] ([cnsctvo_cdgo_tpo_prcso]) ON DELETE CASCADE
);


GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbTipoProceso_Vigencias
...
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbTipoProceso_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbTipoProceso_Vigencias] ON   [dbo].[tbTipoProceso_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbTipoProceso_Vigencias'
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbTipoProceso_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbTipoProceso y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbTipoProceso_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTipoProceso_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTipoProceso_Vigencias_usro_crcn]    ON    [dbo].[tbTipoProceso_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTipoProceso_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTipoProceso_Vigencias]
ON  [dbo].[tbTipoProceso_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTipoProceso_Vigencias ','cdgo_tpo_prcso')) - 1
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbTipoProceso_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbModosAplicacion_Vigencias 		D\>
* 			  <\D Cuando se actualiza un Campo de tbTipoProceso_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbTipoProceso_Vigencias]   On [dbo].[tbTipoProceso_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbTipoProceso_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbTipoProceso_Vigencias',@lcCampo,
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbTipoProceso_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbTipoProceso_Vigencias] ON   [dbo].[tbTipoProceso_Vigencias]
FOR  UPDATE
AS
Begin
...
-->Se Calcula si hay  traslape de fechas
	...
	FROM  tbTipoProceso_Vigencias p
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbTipoProceso_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbTipoProceso_Vigencias] ON [dbo].[tbTipoProceso_Vigencias]
FOR  INSERT
AS
Begin
...
update tbTipoProceso set dscrpcn_tpo_prcso =  @dscrpcn
