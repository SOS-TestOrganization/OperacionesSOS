CREATE TABLE [dbo].[tbGrupoLiquidacionAportante_Vigencias] (
    [cnsctvo_vgnca_grpo_lqdcn_aprtnte] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_grpo_lqdcn_aprtnte]       [dbo].[udtConsecutivo] NOT NULL,
    ...
    CONSTRAINT [PK_TbGrupoLiquidacionAportante_Vigencias] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_grpo_lqdcn_aprtnte] ASC),
    CONSTRAINT [CK_tbGrupoLiquidacionAportante_Vigencias] CHECK ([inco_vgnca] < [fn_vgnca]),
    CONSTRAINT [FK_tbGrupoLiquidacionAportante_Vigencias_tbGrupoLiquidacionAportante] FOREIGN KEY ([cnsctvo_grpo_lqdcn_aprtnte]) REFERENCES [dbo].[tbGrupoLiquidacionAportante] ([cnsctvo_grpo_lqdcn_aprtnte]) ON DELETE CASCADE,
    CONSTRAINT [FK_tbGrupoLiquidacionAportante_Vigencias_TbGruposLiquidacion] FOREIGN KEY ([cnsctvo_cdgo_grpo_lqdcn]) REFERENCES [dbo].[tbGruposLiquidacion] ([cnsctvo_cdgo_grpo_lqdcn])
);


GO
CREATE STATISTICS [cnsctvo_grpo_lqdcn_aprtnte]
    ON [dbo].[tbGrupoLiquidacionAportante_Vigencias]([cnsctvo_grpo_lqdcn_aprtnte], [cnsctvo_vgnca_grpo_lqdcn_aprtnte]);


GO
CREATE STATISTICS [inco_vgnca]
    ON [dbo].[tbGrupoLiquidacionAportante_Vigencias]([inco_vgnca], [cnsctvo_vgnca_grpo_lqdcn_aprtnte]);


GO
CREATE STATISTICS [fn_vgnca]
    ON [dbo].[tbGrupoLiquidacionAportante_Vigencias]([fn_vgnca], [cnsctvo_vgnca_grpo_lqdcn_aprtnte]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbGrupoLiquidacionAportante_Vigencias]([fcha_crcn], [cnsctvo_vgnca_grpo_lqdcn_aprtnte]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbGrupoLiquidacionAportante_Vigencias]([usro_crcn], [cnsctvo_vgnca_grpo_lqdcn_aprtnte]);


GO
CREATE STATISTICS [tme_stmp]
    ON [dbo].[tbGrupoLiquidacionAportante_Vigencias]([tme_stmp]);


GO

/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbGrupoLiquidacionAportante_Vigencias
...
* 			  <\D Cuando se actualiza un Campo de  tbGrupoLiquidacionAportante_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbGrupoLiquidacionAportante_Vigencias]   On [dbo].[tbGrupoLiquidacionAportante_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbGrupoLiquidacionAportante_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 3
						...
						)SELECT 'bdCarteraPac', 'TbGrupoLiquidacionAportante_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_grpo_lqdcn_aprtnte, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_grpo_lqdcn_aprtnte
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_grpo_lqdcn_aprtnte = n.cnsctvo_grpo_lqdcn_aprtnte AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 4
						...
						d.cnsctvo_vgnca_grpo_lqdcn_aprtnte, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_grpo_lqdcn_aprtnte
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_grpo_lqdcn_aprtnte = n.cnsctvo_grpo_lqdcn_aprtnte AND 
							(
...
GO
/*--------------------------------------------------------------------------------------------------------
* Nombre                 	:	TrReferencia_Usuario_tbGrupoLiquidacionAportante_Vigencias
...
*------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_Usuario_tbGrupoLiquidacionAportante_Vigencias] ON    [dbo].[tbGrupoLiquidacionAportante_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------
* Metodo o PRG 	            	 :   TrValidaTraslapeRel_tbGrupoLiquidacionAportante_Vigencias
...
CREATE TRIGGER   [dbo].[TrValidaTraslapeRel_tbGrupoLiquidacionAportante_Vigencias]  ON    [dbo].[tbGrupoLiquidacionAportante_Vigencias]
FOR  INSERT,UPDATE
AS
...
SELECT    @inco_vgnca_Rel = inco_vgnca,
	    ...
	    @cnsctvo_vgnca 					= 	cnsctvo_vgnca_grpo_lqdcn_aprtnte
FROM        INSERTED
...
--Se Calcula si hay  traslape de fechas
 SELECT  @contador   =  count(*)
 FROM	tbGrupoLiquidacionAportante_Vigencias
...
IF @contador > 1
	 ...
	 Begin
		...
		FROM tbGrupoLiquidacionAportante_Vigencias p
		WHERE ( ( convert(varchar(10),inco_vgnca,111)  >=  convert(varchar(10),@inco_vgnca_Rel,111) 
 		And             convert(varchar(10),inco_vgnca,111) <=   convert(varchar(10),@fn_vgnca_Rel,111))   Or
			      (convert(varchar(10),fn_vgnca,111) 	>=   convert(varchar(10),@inco_vgnca_Rel,111) 
		 And             convert(varchar(10),fn_vgnca,111)	<=   convert(varchar(10),@fn_vgnca_Rel,111))   Or
		...
		And   	       nmro_unco_idntfccn_empldr	=  @Identificador_Tabla2_nmro_unco_idntfccn_empldr
		And            cnsctvo_cdgo_clse_aprtnte 	=  @Identificador_Tabla2_cnsctvo_cdgo_clse_aprtnte
  	      	and            p.cnsctvo_vgnca_grpo_lqdcn_aprtnte	!= @cnsctvo_vgnca
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeRel_tbGrupoLiquidacionAportante_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeRel_tbGrupoLiquidacionAportante_Vigencias]    ON   [dbo].[tbGrupoLiquidacionAportante_Vigencias]
FOR  UPDATE
AS
Begin
...
SET NOCOUNT ON
  ...
  --Se capturan las variables que se estan tratando de insertar
	SELECT	@inco_vgnca = inco_vgnca,
			@fn_vgnca	= fn_vgnca,
			@Codigo_Consecutivo = cnsctvo_grpo_lqdcn_aprtnte
	FROM   INSERTED
...
--    Se capturan las variables que se estan modificando
	SELECT     @dinco_vgnca   =  inco_vgnca,
	        		      @dfn_vgnca     =  fn_vgnca,
		     	      @dcnsctvo_tbla = cnsctvo_grpo_lqdcn_aprtnte,
			@cnsctvo_vgnca 	= cnsctvo_vgnca_grpo_lqdcn_aprtnte     
	FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_grpo_lqdcn_aprtnte = n.cnsctvo_grpo_lqdcn_aprtnte)
