CREATE TABLE [dbo].[tbParametrosFijos] (
    ...
    CONSTRAINT [PK_TbParametrosFijos_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_prmtro_fjo] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbParametrosFijos
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbParametrosFijos \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbParametrosFijos] ON    [dbo].[tbParametrosFijos]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbParametrosFijos'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_prmtro_fjo,
	@cdgo = cdgo_prmtro_fjo,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_prmtro_fjo from tbParametrosFijos
	where  cnsctvo_cdgo_prmtro_fjo = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbParametrosFijos', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbParametrosFijos_vigencias where  cnsctvo_cdgo_prmtro_fjo = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbParametrosFijos where  cnsctvo_cdgo_prmtro_fjo = @cnsctvo_tbla
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbParametrosFijos
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbParametrosFijos_usro_crcn]    ON    [dbo].[tbParametrosFijos]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbParametrosFijos
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbParametrosFijos]
ON  [dbo].[tbParametrosFijos]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbParametrosFijos','cdgo_prmtro_fjo')) - 1
