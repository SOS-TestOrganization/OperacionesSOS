CREATE TABLE [dbo].[tbGruposLiquidacion_Vigencias] (
    [cnsctvo_vgnca_grpo_lqdcn] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_grpo_lqdcn]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_grpo_lqdcn]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_grpo_lqdcn]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_TbGruposLiquidacion] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_grpo_lqdcn] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbGruposLiquidacion_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbGruposLiquidacion_Vigencias_TbGruposLiquidacion] FOREIGN KEY ([cnsctvo_cdgo_grpo_lqdcn]) REFERENCES [dbo].[tbGruposLiquidacion] ([cnsctvo_cdgo_grpo_lqdcn]) ON DELETE CASCADE
);
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbGruposLiquidacion_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbGruposLiquidacion_Vigencias] ON   [dbo].[tbGruposLiquidacion_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_grpo_lqdcn	,
		@cdgo_ara = cdgo_grpo_lqdcn		,
		@cdgo_vgnca	= cnsctvo_vgnca_grpo_lqdcn,
		@dscrpcn =  dscrpcn_grpo_lqdcn
FROM   INSERTED
...
-->Se Calcula si hay  traslape de fechas
	...
	And	p.cnsctvo_cdgo_grpo_lqdcn = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_grpo_lqdcn =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_grpo_lqdcn !=  @cdgo_vgnca	
...
update tbGruposLiquidacion set dscrpcn_grpo_lqdcn =  @dscrpcn
where cnsctvo_cdgo_grpo_lqdcn = @Codigo_Consecutivo
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbGruposLiquidacion_Vigencias
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbGruposLiquidacion_Vigencias]
ON  [dbo].[tbGruposLiquidacion_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbGruposLiquidacion_Vigencias ','cdgo_grpo_lqdcn')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_grpo_lqdcn = @lcCodigov  Or  cdgo_grpo_lqdcn =  @lcCodigoi)
Begin
	RollBack Transaction
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbGruposLiquidacion_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbGruposLiquidacion_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbGruposLiquidacion_Vigencias] ON   [dbo].[tbGruposLiquidacion_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbGruposLiquidacion_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_grpo_lqdcn
from INSERTED
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_grpo_lqdcn = n.cnsctvo_vgnca_grpo_lqdcn)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbGruposLiquidacion_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbGruposLiquidacion y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbGruposLiquidacion_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbGruposLiquidacion_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbGruposLiquidacion_Vigencias]   On [dbo].[tbGruposLiquidacion_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbGruposLiquidacion_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbGruposLiquidacion_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_grpo_lqdcn, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_grpo_lqdcn
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_grpo_lqdcn = n.cnsctvo_cdgo_grpo_lqdcn AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_grpo_lqdcn, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_grpo_lqdcn
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_grpo_lqdcn = n.cnsctvo_cdgo_grpo_lqdcn AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbGruposLiquidacion_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbGruposLiquidacion_Vigencias] ON   [dbo].[tbGruposLiquidacion_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_grpo_lqdcn,
	      @cnsctvo_vgnca = cnsctvo_vgnca_grpo_lqdcn			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_grpo_lqdcn = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_grpo_lqdcn != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbGruposLiquidacion_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbGruposLiquidacion_Vigencias_usro_crcn]    ON    [dbo].[tbGruposLiquidacion_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
