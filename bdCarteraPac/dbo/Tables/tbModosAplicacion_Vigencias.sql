CREATE TABLE [dbo].[tbModosAplicacion_Vigencias] (
    [cnsctvo_vgnca_mdo_aplccn] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_mdo_aplccn]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_mdo_aplccn]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_mdo_aplccn]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_tbModosAplicacion] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_mdo_aplccn] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbModosAplicacion_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbModosAplicacion_Vigencias_tbModosAplicacion] FOREIGN KEY ([cnsctvo_cdgo_mdo_aplccn]) REFERENCES [dbo].[tbModosAplicacion] ([cnsctvo_cdgo_mdo_aplccn]) ON DELETE CASCADE
);
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbModosAplicacion_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbModosAplicacion_Vigencias] ON   [dbo].[tbModosAplicacion_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_mdo_aplccn	,
		@cdgo_ara = cdgo_mdo_aplccn		,
		@cdgo_vgnca	= cnsctvo_vgnca_mdo_aplccn,
		@dscrpcn =  dscrpcn_mdo_aplccn
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbModosAplicacion_Vigencias p
	...
	And	p.cnsctvo_cdgo_mdo_aplccn = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_mdo_aplccn =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_mdo_aplccn !=  @cdgo_vgnca	
...
update tbModosAplicacion set dscrpcn_mdo_aplccn =  @dscrpcn
where cnsctvo_cdgo_mdo_aplccn = @Codigo_Consecutivo
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbModosAplicacion_Vigencias
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbModosAplicacion_Vigencias]
ON  [dbo].[tbModosAplicacion_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbModosAplicacion_Vigencias ','cdgo_mdo_aplccn')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_mdo_aplccn = @lcCodigov  Or  cdgo_mdo_aplccn =  @lcCodigoi)
Begin
	RollBack Transaction
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbModosAplicacion_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbModosAplicacion_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbModosAplicacion_Vigencias] ON   [dbo].[tbModosAplicacion_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbModosAplicacion_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_mdo_aplccn
from INSERTED
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_mdo_aplccn = n.cnsctvo_vgnca_mdo_aplccn)
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbModosAplicacion_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbModosAplicacion y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbModosAplicacion_Vigencias  y  ' + @lcopcion_menu	
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbModosAplicacion_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbModosAplicacion_Vigencias_usro_crcn]    ON    [dbo].[tbModosAplicacion_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbModosAplicacion_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbModosAplicacion_Vigencias 		D\>
* 			  <\D Cuando se actualiza un Campo de tbGruposLiquidacion_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbModosAplicacion_Vigencias]   On [dbo].[tbModosAplicacion_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbModosAplicacion_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbModosAplicacion_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_mdo_aplccn, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_mdo_aplccn
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_mdo_aplccn = n.cnsctvo_cdgo_mdo_aplccn AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_mdo_aplccn, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_mdo_aplccn
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_mdo_aplccn = n.cnsctvo_cdgo_mdo_aplccn AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbModosAplicacion_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbModosAplicacion_Vigencias] ON   [dbo].[tbModosAplicacion_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_mdo_aplccn,
	      @cnsctvo_vgnca = cnsctvo_vgnca_mdo_aplccn			     
FROM	deleted
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_mdo_aplccn = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_mdo_aplccn != @cnsctvo_vgnca
