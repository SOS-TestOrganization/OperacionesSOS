CREATE TABLE [dbo].[tbTiposMorosidad] (
    ...
    CONSTRAINT [PK_tbTiposMorosidad_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_tpo_mrsdd] ASC)
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_tbTiposMorosidad]
    ON [dbo].[tbTiposMorosidad]([cdgo_tpo_mrsdd] ASC);


GO
CREATE STATISTICS [dscrpcn_tpo_mrsdd]
    ON [dbo].[tbTiposMorosidad]([dscrpcn_tpo_mrsdd], [cnsctvo_cdgo_tpo_mrsdd]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbTiposMorosidad]([fcha_crcn], [cnsctvo_cdgo_tpo_mrsdd]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbTiposMorosidad]([usro_crcn], [cnsctvo_cdgo_tpo_mrsdd]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbTiposMorosidad]([vsble_usro], [cnsctvo_cdgo_tpo_mrsdd]);


GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposMorosidad
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposMorosidad]
ON  [dbo].[tbTiposMorosidad]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposMorosidad','cdgo_tpo_mrsdd')) - 1
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbTiposMorosidad
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbTiposMorosidad \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbTiposMorosidad] ON    [dbo].[tbTiposMorosidad]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbTiposMorosidad'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_tpo_mrsdd,
	@cdgo = cdgo_tpo_mrsdd,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_tpo_mrsdd from tbTiposMorosidad
	where  cnsctvo_cdgo_tpo_mrsdd = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbTiposMorosidad', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbTiposMorosidad_vigencias where  cnsctvo_cdgo_tpo_mrsdd = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbTiposMorosidad where  cnsctvo_cdgo_tpo_mrsdd = @cnsctvo_tbla
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposMorosidad
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposMorosidad_usro_crcn]    ON    [dbo].[tbTiposMorosidad]
FOR  INSERT,UPDATE
AS
Begin
