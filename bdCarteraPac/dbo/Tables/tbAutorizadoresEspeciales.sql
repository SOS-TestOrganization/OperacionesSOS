CREATE TABLE [dbo].[tbAutorizadoresEspeciales] (
    ...
    CONSTRAINT [PK_tbAutorizadoresEspeciales_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_autrzdr_espcl] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbAutorizadoresEspeciales
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbAutorizadoresEspeciales \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbAutorizadoresEspeciales] ON    [dbo].[tbAutorizadoresEspeciales]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbAutorizadoresEspeciales'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_autrzdr_espcl,
	@cdgo = cdgo_autrzdr_espcl,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_autrzdr_espcl from tbAutorizadoresEspeciales
	where  cnsctvo_cdgo_autrzdr_espcl = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbAutorizadoresEspeciales', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbAutorizadoresEspeciales_vigencias where  cnsctvo_cdgo_autrzdr_espcl = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbAutorizadoresEspeciales where  cnsctvo_cdgo_autrzdr_espcl = @cnsctvo_tbla
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbAutorizadoresEspeciales
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbAutorizadoresEspeciales_usro_crcn]    ON    [dbo].[tbAutorizadoresEspeciales]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbAutorizadoresEspeciales
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbAutorizadoresEspeciales]
ON  [dbo].[tbAutorizadoresEspeciales]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbAutorizadoresEspeciales','cdgo_autrzdr_espcl')) - 1
