CREATE TABLE [dbo].[tbResponsableXDescuento_Vigencias] (
    [cnsctvo_vgnca_rspnsble_dscnto] INT                             NOT NULL,
    ...
    [cnsctvo_cdgo_tpo_idntfccn]     [dbo].[udtConsecutivo]          NULL,
    [tme_stmp]                      ROWVERSION                      NOT NULL,
    CONSTRAINT [PK_TbResponsableXDescuento] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_rspnsble_dscnto] ASC) WITH (FILLFACTOR = 90),
    CONSTRAINT [CK_tbResponsableXDescuento_Vigencias] CHECK ([inco_vgnca] < [fn_vgnca]),
    CONSTRAINT [FK_tbResponsableXDescuento_Vigencias_tbAutorizadoresEspeciales] FOREIGN KEY ([cnsctvo_cdgo_autrzdr_espcl]) REFERENCES [dbo].[tbAutorizadoresEspeciales] ([cnsctvo_cdgo_autrzdr_espcl]),
    CONSTRAINT [FK_tbResponsableXDescuento_Vigencias_tbBasesAplicadas] FOREIGN KEY ([cnsctvo_cdgo_bse_aplcda]) REFERENCES [dbo].[tbBasesAplicadas] ([cnsctvo_cdgo_bse_aplcda]),
    CONSTRAINT [FK_tbResponsableXDescuento_Vigencias_tbDescuentos] FOREIGN KEY ([cnsctvo_cdgo_dscnto]) REFERENCES [dbo].[tbDescuentos] ([cnsctvo_cdgo_dscnto]),
    CONSTRAINT [FK_tbResponsableXDescuento_Vigencias_tbResponsableXDescuento] FOREIGN KEY ([cnsctvo_cdgo_rspnsble_dscnto]) REFERENCES [dbo].[tbResponsableXDescuento] ([cnsctvo_cdgo_rspnsble_dscnto]) ON DELETE CASCADE
);


GO
/*--------------------------------------------------------------------------------------------------------
* Nombre                 	:	TrReferencia_Usuario_tbResponsableXDescuento_Vigencias
...
*------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_Usuario_tbResponsableXDescuento_Vigencias] ON    [dbo].[tbResponsableXDescuento_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------
* Metodo o PRG 	            	 :   TrValidaTraslapeRel_tbResponsableXDescuento_Vigencias
...
CREATE TRIGGER   [dbo].[TrValidaTraslapeRel_tbResponsableXDescuento_Vigencias]  ON    [dbo].[tbResponsableXDescuento_Vigencias]
FOR  INSERT,UPDATE
AS
declare @Identificador_Tabla1					udtconsecutivo,
	@Identificador_Tabla2_nmro_unco_idntfccn		udtconsecutivo,
...
SELECT    @inco_vgnca_Rel = inco_vgnca,
	    ...
	    @Identificador_Tabla1= cnsctvo_cdgo_dscnto, 
	    @Identificador_Tabla2_nmro_unco_idntfccn		=	nmro_unco_idntfccn,
	    @Identificador_Tabla2_cnsctvo_scrsl			=	cnsctvo_scrsl,
	    @Identificador_Tabla2_cnsctvo_cdgo_clse_aprtnte	=	cnsctvo_cdgo_clse_aprtnte,	
	    @cnsctvo_vgnca 					= 	cnsctvo_vgnca_rspnsble_dscnto
FROM        INSERTED
...
SELECT @contador=count(*)
	...
	And	p.cnsctvo_cdgo_dscnto = @Identificador_Tabla1
...
If    @contador<1     
	 Begin
	      ROLLBACK TRANSACTION
	      select @mnsje  =  'Error, no cumple con el rango de fechas en el Parámetro tbDescuentos_Vigencias'  
...
-- no se debe de tener en cuenta porque  cuando aplica a nivel de contrato toma la cucursal cero y la clase aportante cero.
/*
set @contador=0
SELECT @contador=count(*)
	FROM bdAfiliacion..tbSucursalesAportante p
	WHERE cnsctvo_scrsl		=  @Identificador_Tabla2_cnsctvo_scrsl		
	And   nmro_unco_idntfccn_empldr	=  @Identificador_Tabla2_nmro_unco_idntfccn
...
--Se Calcula si hay  traslape de fechas
 SELECT  @contador   =  count(*)
 FROM	tbResponsableXDescuento_Vigencias
...
And            cnsctvo_cdgo_dscnto 		=  @Identificador_Tabla1
And	       cnsctvo_scrsl			=  @Identificador_Tabla2_cnsctvo_scrsl		
And   	       nmro_unco_idntfccn		=  @Identificador_Tabla2_nmro_unco_idntfccn
And            cnsctvo_cdgo_clse_aprtnte 	=  @Identificador_Tabla2_cnsctvo_cdgo_clse_aprtnte
...
IF @contador > 1
	 ...
	 Begin
		...
		FROM tbResponsableXDescuento_Vigencias p
		WHERE ( ( convert(varchar(10),inco_vgnca,111)  >=  convert(varchar(10),@inco_vgnca_Rel,111) 
 		And             convert(varchar(10),inco_vgnca,111) <=   convert(varchar(10),@fn_vgnca_Rel,111))   Or
			      (convert(varchar(10),fn_vgnca,111) 	>=   convert(varchar(10),@inco_vgnca_Rel,111) 
		 And             convert(varchar(10),fn_vgnca,111)	<=   convert(varchar(10),@fn_vgnca_Rel,111))   Or
		...
		And   	       nmro_unco_idntfccn		=  @Identificador_Tabla2_nmro_unco_idntfccn
		And            cnsctvo_cdgo_clse_aprtnte 	=  @Identificador_Tabla2_cnsctvo_cdgo_clse_aprtnte
  	      	and            p.cnsctvo_vgnca_rspnsble_dscnto	!= @cnsctvo_vgnca
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeRel_tbResponsableXDescuento_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeRel_tbResponsableXDescuento_Vigencias]    ON   [dbo].[tbResponsableXDescuento_Vigencias]
FOR  UPDATE
AS
Begin
...
SET NOCOUNT ON
  ...
  --Se capturan las variables que se estan tratando de insertar
	SELECT	@inco_vgnca = inco_vgnca,
			@fn_vgnca	= fn_vgnca,
			@Codigo_Consecutivo = cnsctvo_cdgo_rspnsble_dscnto
	FROM   INSERTED
...
--    Se capturan las variables que se estan modificando
	SELECT     @dinco_vgnca   =  inco_vgnca,
	        		      @dfn_vgnca     =  fn_vgnca,
		     	      @dcnsctvo_tbla = cnsctvo_cdgo_rspnsble_dscnto,
			@cnsctvo_vgnca 	= cnsctvo_vgnca_rspnsble_dscnto     
	FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_cdgo_rspnsble_dscnto = n.cnsctvo_cdgo_rspnsble_dscnto)
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbResponsableXDescuento_Vigencias
...
* 			  <\D Cuando se actualiza un Campo de tbResponsableXDescuento_Vigencias D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbResponsableXDescuento_Vigencias]  On [dbo].[tbResponsableXDescuento_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbResponsableXDescuento_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'tbResponsableXDescuento_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_rspnsble_dscnto, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_rspnsble_dscnto
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_rspnsble_dscnto = n.cnsctvo_cdgo_rspnsble_dscnto AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_rspnsble_dscnto, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_rspnsble_dscnto
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_rspnsble_dscnto = n.cnsctvo_cdgo_rspnsble_dscnto AND 
							(
