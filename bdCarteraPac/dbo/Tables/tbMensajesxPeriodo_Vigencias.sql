CREATE TABLE [dbo].[tbMensajesxPeriodo_Vigencias] (
    [cnsctvo_vgnca_mnsje_prdo] [dbo].[udtConsecutivo] NOT NULL,
    ...
    CONSTRAINT [PK_TbMensajesxPeriodo] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_mnsje_prdo] ASC),
    CONSTRAINT [CK_tbMensajesxPeriodo_Vigencias] CHECK ([inco_vgnca] < [fn_vgnca]),
    CONSTRAINT [FK_tbMensajesxPeriodo_Vigencias_tbMensajes] FOREIGN KEY ([cnsctvo_cdgo_mnsje]) REFERENCES [dbo].[tbMensajes] ([cnsctvo_cdgo_mnsje]),
    CONSTRAINT [FK_tbMensajesxPeriodo_Vigencias_tbMensajesxPeriodo] FOREIGN KEY ([cnsctvo_mnsje_prdo]) REFERENCES [dbo].[tbMensajesxPeriodo] ([cnsctvo_mnsje_prdo]) ON DELETE CASCADE
);


GO
/*--------------------------------------------------------------------------------------------------------
* Nombre                 	:	TrReferencia_Usuario_tbMensajesxPeriodo_Vigencias
...
*------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_Usuario_tbMensajesxPeriodo_Vigencias] ON    [dbo].[tbMensajesxPeriodo_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------
* Metodo o PRG 	            	 :   TrValidaTraslapeRel_tbMensajesxPeriodo_Vigencias
...
CREATE TRIGGER   [dbo].[TrValidaTraslapeRel_tbMensajesxPeriodo_Vigencias]  ON    [dbo].[tbMensajesxPeriodo_Vigencias]
FOR  INSERT,UPDATE
AS
...
SELECT    @inco_vgnca_Rel = inco_vgnca,
	    ...
	    @Identificador_Tabla1= cnsctvo_cdgo_prdo_lqdcn, 
	    @Identificador_Tabla2  = cnsctvo_cdgo_mnsje,
	    @cnsctvo_vgnca = cnsctvo_vgnca_mnsje_prdo
FROM        INSERTED
...
SELECT @contador=count(*)
	...
	And	p.cnsctvo_cdgo_prdo_lqdcn = @Identificador_Tabla1
...
If    @contador<1     
	 Begin
	      ROLLBACK TRANSACTION
	      select @mnsje  =  'Error, no cumple con el rango de fechas en el Parámetro tbPeriodosliquidacion_Vigencias'  
...
SELECT @contador=count(*)
	...
	WHERE convert(varchar(10),p.inco_vgnca,111)  <= convert(varchar(10),@inco_vgnca_Rel,111)  and 
		convert(varchar(10),p.fn_vgnca,111)    >= convert(varchar(10),@fn_vgnca_Rel,111) 
	And	p.cnsctvo_cdgo_mnsje  =  @Identificador_Tabla2
...
If    @contador<1      
	 Begin
	      ROLLBACK TRANSACTION
	      select @mnsje  =  'Error, no cumple con el rango de fechas en el Parámetro tbMensajes_Vigencias '  
	      RAISERROR (999901, 16, 1,  @mnsje)
	      RETURN
	 End
...
--Se Calcula si hay  traslape de fechas
 SELECT  @contador   =  count(*)
 FROM	tbMensajesxPeriodo_Vigencias
 ...
 And           cnsctvo_cdgo_prdo_lqdcn = @Identificador_Tabla1
 And           cnsctvo_cdgo_mnsje = @Identificador_Tabla2
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeRel_tbMensajesxPeriodo_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeRel_tbMensajesxPeriodo_Vigencias]    ON   [dbo].[tbMensajesxPeriodo_Vigencias]
FOR  UPDATE
AS
Begin
...
SET NOCOUNT ON
  ...
  --Se capturan las variables que se estan tratando de insertar
	SELECT	@inco_vgnca = inco_vgnca,
			@fn_vgnca	= fn_vgnca,
			@Codigo_Consecutivo = cnsctvo_mnsje_prdo
	FROM   INSERTED
...
--    Se capturan las variables que se estan modificando
	SELECT     @dinco_vgnca   =  inco_vgnca,
	        		      @dfn_vgnca     =  fn_vgnca,
		     	      @dcnsctvo_tbla = cnsctvo_mnsje_prdo,
			@cnsctvo_vgnca = cnsctvo_vgnca_mnsje_prdo     
	FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_mnsje_prdo = n.cnsctvo_mnsje_prdo)
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbMensajesxPeriodo_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbMensajesxPeriodo_Vigencias 		D\>
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrIncidencias_tbMensajesxPeriodo_Vigencias]   On [dbo].[tbMensajesxPeriodo_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbMensajesxPeriodo_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 3
						...
						)SELECT 'bdCarteraPac', 'tbMensajesxPeriodo_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_mnsje_prdo, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_mnsje_prdo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_mnsje_prdo = n.cnsctvo_mnsje_prdo AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 4
						...
						d.cnsctvo_vgnca_mnsje_prdo, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_mnsje_prdo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_mnsje_prdo = n.cnsctvo_mnsje_prdo AND 
							(
