CREATE TABLE [dbo].[tbGruposxConcepto] (
    ...
    CONSTRAINT [PK_TbGruposxConcepto_1] PRIMARY KEY CLUSTERED ([cnsctvo_grpo_cncpto] ASC) WITH (FILLFACTOR = 90),
    CONSTRAINT [FK_tbGruposxConcepto_tbConceptosliquidacion] FOREIGN KEY ([cnsctvo_cdgo_cncpto_lqdcn]) REFERENCES [dbo].[tbConceptosLiquidacion] ([cnsctvo_cdgo_cncpto_lqdcn]),
    CONSTRAINT [FK_tbGruposxConcepto_TbGruposLiquidacion] FOREIGN KEY ([cnsctvo_cdgo_grpo_lqdcn]) REFERENCES [dbo].[tbGruposLiquidacion] ([cnsctvo_cdgo_grpo_lqdcn])
);


GO
/*--------------------------------------------------------------------------------------------------------
* Nombre                 	:	TrReferencia_Usuario_tbGruposxConcepto
...
*------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_Usuario_tbGruposxConcepto]  ON    [dbo].[tbGruposxConcepto] 
FOR  INSERT,UPDATE
AS
Begin
