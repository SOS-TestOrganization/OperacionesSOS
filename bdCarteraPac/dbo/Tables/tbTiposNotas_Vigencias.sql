CREATE TABLE [dbo].[tbTiposNotas_Vigencias] (
    [cnsctvo_vgnca_tpo_nta] CHAR (10)              NOT NULL,
    [cnsctvo_cdgo_tpo_nta]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_tpo_nta]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_tpo_nta]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_tbTiposNotas] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_tpo_nta] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbTiposNotas_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbTiposNotas_Vigencias_tbTiposNotas] FOREIGN KEY ([cnsctvo_cdgo_tpo_nta]) REFERENCES [dbo].[tbTiposNotas] ([cnsctvo_cdgo_tpo_nta]) ON DELETE CASCADE
);
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbTiposNotas_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbTiposNotas_Vigencias] ON   [dbo].[tbTiposNotas_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_tpo_nta	,
		@cdgo_ara = cdgo_tpo_nta		,
		@cdgo_vgnca	= cnsctvo_vgnca_tpo_nta,
		@dscrpcn =  dscrpcn_tpo_nta
FROM   INSERTED
...
-->Se Calcula si hay  traslape de fechas
	...
	And	p.cnsctvo_cdgo_tpo_nta = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_tpo_nta =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_tpo_nta !=  @cdgo_vgnca	
...
update tbTiposNotas set dscrpcn_tpo_nta =  @dscrpcn
where cnsctvo_cdgo_tpo_nta = @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbTiposNotas_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbModosAplicacion_Vigencias 		D\>
* 			  <\D Cuando se actualiza un Campo de  tbTiposNotas_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbTiposNotas_Vigencias]   On [dbo].[tbTiposNotas_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbTiposNotas_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbTiposNotas_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_tpo_nta, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_tpo_nta
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_tpo_nta = n.cnsctvo_cdgo_tpo_nta AND 
							(
							 (
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbTiposNotas_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbTiposNotas_Vigencias] ON   [dbo].[tbTiposNotas_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_tpo_nta,
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_tpo_nta = n.cnsctvo_vgnca_tpo_nta )
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbTiposNotas_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbTiposNotas_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbTiposNotas_Vigencias] ON   [dbo].[tbTiposNotas_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbTiposNotas_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_tpo_nta
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbTiposNotas_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbTiposNotas y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbTiposNotas_Vigencias  y  ' + @lcopcion_menu	
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposNotas_Vigencias
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposNotas_Vigencias]
ON  [dbo].[tbTiposNotas_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposNotas_Vigencias ','cdgo_tpo_nta')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_tpo_nta = @lcCodigov  Or  cdgo_tpo_nta =  @lcCodigoi)
Begin
	RollBack Transaction
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposNotas_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposNotas_Vigencias_usro_crcn]    ON    [dbo].[tbTiposNotas_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
