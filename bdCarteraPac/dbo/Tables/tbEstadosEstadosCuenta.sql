CREATE TABLE [dbo].[tbEstadosEstadosCuenta] (
    [cnsctvo_cdgo_estdo_estdo_cnta] [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_estdo_estdo_cnta]         [dbo].[udtCodigo]      NULL,
    [dscrpcn_estdo_estdo_cnta]      [dbo].[udtDescripcion] NULL,
    [fcha_crcn]                     DATETIME               NULL,
    [usro_crcn]                     [dbo].[udtUsuario]     NULL,
    [vsble_usro]                    [dbo].[udtLogico]      NULL,
    CONSTRAINT [PK_tbEstadosEstadosCuenta_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_estdo_estdo_cnta] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbEstadosEstadosCuenta
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbEstadosEstadosCuenta \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbEstadosEstadosCuenta] ON    [dbo].[tbEstadosEstadosCuenta]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbEstadosEstadosCuenta'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_estdo_estdo_cnta,
	@cdgo = cdgo_estdo_estdo_cnta,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_estdo_estdo_cnta from tbEstadosEstadosCuenta
	where  cnsctvo_cdgo_estdo_estdo_cnta = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbEstadosEstadosCuenta', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbEstadosEstadosCuenta_vigencias where  cnsctvo_cdgo_estdo_estdo_cnta = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbEstadosEstadosCuenta where  cnsctvo_cdgo_estdo_estdo_cnta = @cnsctvo_tbla
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosEstadosCuenta
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosEstadosCuenta_usro_crcn]    ON    [dbo].[tbEstadosEstadosCuenta]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosEstadosCuenta
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosEstadosCuenta]
ON  [dbo].[tbEstadosEstadosCuenta]
FOR  UPDATE, DELETE 
AS 
...
Set	@lcCodigoi ='9'
/*
Select	@lcLongitud = (COL_LENGTH('tbEstadosEstadosCuenta','cdgo_estdo_estdo_cnta')) - 1
