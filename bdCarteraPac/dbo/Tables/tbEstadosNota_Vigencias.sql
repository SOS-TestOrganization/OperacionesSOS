CREATE TABLE [dbo].[tbEstadosNota_Vigencias] (
    [cnsctvo_vgnca_estdo_nta] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_estdo_nta]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_estdo_nta]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_estdo_nta]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_tbEstadosNota] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_estdo_nta] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbEstadosNota_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbEstadosNota_Vigencias_tbEstadosNota] FOREIGN KEY ([cnsctvo_cdgo_estdo_nta]) REFERENCES [dbo].[tbEstadosNota] ([cnsctvo_cdgo_estdo_nta]) ON DELETE CASCADE
);


GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosNota_Vigencias
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosNota_Vigencias]
ON  [dbo].[tbEstadosNota_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstadosNota_Vigencias ','cdgo_estdo_nta')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_estdo_nta = @lcCodigov  Or  cdgo_estdo_nta =  @lcCodigoi)
Begin
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbEstadosNota_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbEstadosNota_Vigencias] ON   [dbo].[tbEstadosNota_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_estdo_nta	,
		@cdgo_ara = cdgo_estdo_nta		,
		@cdgo_vgnca	= cnsctvo_vgnca_estdo_nta,
		@dscrpcn =  dscrpcn_estdo_nta
FROM   INSERTED
...
-->Se Calcula si hay  traslape de fechas
	...
	And	p.cnsctvo_cdgo_estdo_nta = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_estdo_nta =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_estdo_nta !=  @cdgo_vgnca	
...
update tbEstadosNota set dscrpcn_estdo_nta =  @dscrpcn
where cnsctvo_cdgo_estdo_nta = @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbEstadosNota_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbEstadosNota_Vigencias 		D\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbEstadosNota_Vigencias]   On [dbo].[tbEstadosNota_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbEstadosNota_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'tbEstadosNota_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_estdo_nta, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_estdo_nta
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_estdo_nta = n.cnsctvo_cdgo_estdo_nta AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_estdo_nta, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_estdo_nta
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_estdo_nta = n.cnsctvo_cdgo_estdo_nta AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbEstadosNota_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbEstadosNota_Vigencias] ON   [dbo].[tbEstadosNota_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_estdo_nta,
	      @cnsctvo_vgnca = cnsctvo_vgnca_estdo_nta			     
FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_estdo_nta = n.cnsctvo_vgnca_estdo_nta )
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_estdo_nta = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_estdo_nta != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosNota_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosNota_Vigencias_usro_crcn]    ON    [dbo].[tbEstadosNota_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbEstadosNota_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbEstadosNota_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbEstadosNota_Vigencias] ON   [dbo].[tbEstadosNota_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbEstadosNota_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_estdo_nta
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbEstadosNota_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbEstadosNota y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbEstadosNota_Vigencias  y  ' + @lcopcion_menu	
