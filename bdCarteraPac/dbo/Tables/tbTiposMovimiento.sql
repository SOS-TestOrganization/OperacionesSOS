CREATE TABLE [dbo].[tbTiposMovimiento] (
    ...
    CONSTRAINT [PK_tbTiposMovimiento_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_tpo_mvmnto] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbTiposMovimiento
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbTiposMovimiento \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbTiposMovimiento] ON    [dbo].[tbTiposMovimiento]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbTiposMovimiento'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_tpo_mvmnto,
	@cdgo = cdgo_tpo_mvmnto,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_tpo_mvmnto from tbTiposMovimiento
	where  cnsctvo_cdgo_tpo_mvmnto = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbTiposMovimiento', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbTiposMovimiento_vigencias where  cnsctvo_cdgo_tpo_mvmnto = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbTiposMovimiento where  cnsctvo_cdgo_tpo_mvmnto = @cnsctvo_tbla
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposMovimiento
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposMovimiento_usro_crcn]    ON    [dbo].[tbTiposMovimiento]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposMovimiento
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposMovimiento]
ON  [dbo].[tbTiposMovimiento]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposMovimiento','cdgo_tpo_mvmnto')) - 1
