CREATE TABLE [dbo].[tbEstadosPago_Vigencias] (
    [cnsctvo_vgnca_estdo_pgo] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_estdo_pgo]  [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_estdo_pgo]          [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_estdo_pgo]       [dbo].[udtDescripcion] NOT NULL,
    ...
    CONSTRAINT [PK_tbEstadosPago] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_estdo_pgo] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbEstadosPago_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbEstadosPago_Vigencias_tbEstadosPago] FOREIGN KEY ([cnsctvo_cdgo_estdo_pgo]) REFERENCES [dbo].[tbEstadosPago] ([cnsctvo_cdgo_estdo_pgo]) ON DELETE CASCADE
);


GO
CREATE STATISTICS [cnsctvo_cdgo_estdo_pgo]
    ON [dbo].[tbEstadosPago_Vigencias]([cnsctvo_cdgo_estdo_pgo], [cnsctvo_vgnca_estdo_pgo]);


GO
CREATE STATISTICS [cdgo_estdo_pgo]
    ON [dbo].[tbEstadosPago_Vigencias]([cdgo_estdo_pgo], [cnsctvo_vgnca_estdo_pgo]);


GO
CREATE STATISTICS [dscrpcn_estdo_pgo]
    ON [dbo].[tbEstadosPago_Vigencias]([dscrpcn_estdo_pgo], [cnsctvo_vgnca_estdo_pgo]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbEstadosPago_Vigencias]([fcha_crcn], [cnsctvo_vgnca_estdo_pgo]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbEstadosPago_Vigencias]([usro_crcn], [cnsctvo_vgnca_estdo_pgo]);


GO
CREATE STATISTICS [obsrvcns]
    ON [dbo].[tbEstadosPago_Vigencias]([obsrvcns], [cnsctvo_vgnca_estdo_pgo]);


GO
CREATE STATISTICS [tme_stmp]
    ON [dbo].[tbEstadosPago_Vigencias]([tme_stmp]);


GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbEstadosPago_Vigencias
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbEstadosPago_Vigencias]
ON  [dbo].[tbEstadosPago_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbEstadosPago_Vigencias ','cdgo_estdo_pgo')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_estdo_pgo = @lcCodigov  Or  cdgo_estdo_pgo =  @lcCodigoi)
Begin
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbEstadosPago_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbEstadosPago_Vigencias] ON   [dbo].[tbEstadosPago_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_estdo_pgo	,
		@cdgo_ara = cdgo_estdo_pgo		,
		@cdgo_vgnca	= cnsctvo_vgnca_estdo_pgo,
		@dscrpcn =  dscrpcn_estdo_pgo
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbEstadosPago_Vigencias p
	...
	And	p.cnsctvo_cdgo_estdo_pgo = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_estdo_pgo =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_estdo_pgo !=  @cdgo_vgnca	
...
update tbEstadosPago set dscrpcn_estdo_pgo =  @dscrpcn
where cnsctvo_cdgo_estdo_pgo = @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbEstadosPago_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbIncidenciasCarteraPac 		D\>
* 			  <\D Cuando se actualiza un Campo de  tbEstadosPago_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbEstadosPago_Vigencias]   On [dbo].[tbEstadosPago_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.TbEstadosPago_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'TbEstadosPago_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_estdo_pgo, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_estdo_pgo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_estdo_pgo = n.cnsctvo_cdgo_estdo_pgo AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_estdo_pgo, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_estdo_pgo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_estdo_pgo = n.cnsctvo_cdgo_estdo_pgo AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbEstadosPago_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbEstadosPago_Vigencias] ON   [dbo].[tbEstadosPago_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_estdo_pgo,
	      @cnsctvo_vgnca = cnsctvo_vgnca_estdo_pgo			     
FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_estdo_pgo = n.cnsctvo_vgnca_estdo_pgo )
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_estdo_pgo = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_estdo_pgo != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbEstadosPago_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbEstadosPago_Vigencias_usro_crcn]    ON    [dbo].[tbEstadosPago_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbEstadosPago_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbEstadosPago_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbEstadosPago_Vigencias] ON   [dbo].[tbEstadosPago_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbEstadosPago_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_estdo_pgo
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbEstadosPago_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbEstadosPago y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbEstadosPago_Vigencias  y  ' + @lcopcion_menu	
