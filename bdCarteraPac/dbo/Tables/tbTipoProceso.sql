CREATE TABLE [dbo].[tbTipoProceso] (
    ...
    CONSTRAINT [PK_TbTipoProceso1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_tpo_prcso] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbTipoProceso
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbTipoProceso \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbTipoProceso] ON    [dbo].[tbTipoProceso]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbTipoProceso'
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_tpo_prcso from tbTipoProceso
	where  cnsctvo_cdgo_tpo_prcso = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbTipoProceso', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbTipoProceso_vigencias where  cnsctvo_cdgo_tpo_prcso = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbTipoProceso where  cnsctvo_cdgo_tpo_prcso = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTipoProceso
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTipoProceso]
ON  [dbo].[tbTipoProceso]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTipoProceso','cdgo_tpo_prcso')) - 1
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTipoProceso
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTipoProceso_usro_crcn]    ON    [dbo].[tbTipoProceso]
FOR  INSERT,UPDATE
AS
Begin
