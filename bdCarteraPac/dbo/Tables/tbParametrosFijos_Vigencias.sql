CREATE TABLE [dbo].[tbParametrosFijos_Vigencias] (
    [cnsctvo_vgnca_prmtro_fjo]      [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_cdgo_prmtro_fjo]       [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_prmtro_fjo]               [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_prmtro_fjo]            [dbo].[udtDescripcion] NOT NULL,
    ...
    [cnsctvo_tpo_cncpto_intrs_mnsl] [dbo].[udtConsecutivo] NOT NULL,
    [cnsctvo_tpo_cntrto_pc]         [dbo].[udtConsecutivo] NOT NULL,
    [tme_stmp]                      ROWVERSION             NOT NULL,
    CONSTRAINT [PK_TbParametrosFijos] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_prmtro_fjo] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbParametrosFijos_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbParametrosFijos_Vigencias_tbConceptosliquidacion] FOREIGN KEY ([cnsctvo_tpo_cncpto_intrs_mnsl]) REFERENCES [dbo].[tbConceptosLiquidacion] ([cnsctvo_cdgo_cncpto_lqdcn]),
    CONSTRAINT [FK_tbParametrosFijos_Vigencias_tbParametrosFijos] FOREIGN KEY ([cnsctvo_cdgo_prmtro_fjo]) REFERENCES [dbo].[tbParametrosFijos] ([cnsctvo_cdgo_prmtro_fjo]) ON DELETE CASCADE
);


GO
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbParametrosFijos_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbParametrosFijos_Vigencias] ON   [dbo].[tbParametrosFijos_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo =  cnsctvo_cdgo_prmtro_fjo	,
		@cdgo_ara =  cdgo_prmtro_fjo		
FROM   INSERTED

-->    Se capturan las variables que se estan modificando
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_prmtro_fjo,
	      @cnsctvo_vgnca = cnsctvo_vgnca_prmtro_fjo			     
FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_prmtro_fjo = n.cnsctvo_vgnca_prmtro_fjo )
...
-->Se Calcula si hay  traslape de fechas
	...
	FROM  tbParametrosFijos_Vigencias p
	...
	And	p.cnsctvo_cdgo_prmtro_fjo = @Codigo_Consecutivo
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_prmtro_fjo = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_prmtro_fjo != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbParametrosFijos_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbParametrosFijos_Vigencias 		D\>
* 			  <\D Cuando se actualiza un Campo de tbMensajes_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbParametrosFijos_Vigencias]   On [dbo].[tbParametrosFijos_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbParametrosFijos_Vigencias'
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'tbParametrosFijos_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_prmtro_fjo, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_prmtro_fjo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_prmtro_fjo = n.cnsctvo_cdgo_prmtro_fjo AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_prmtro_fjo, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_prmtro_fjo
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_prmtro_fjo = n.cnsctvo_cdgo_prmtro_fjo AND 
							(
...
GO
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbParametrosFijos_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbParametrosFijos_Vigencias_usro_crcn]    ON    [dbo].[tbParametrosFijos_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbParametrosFijos_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbParametrosFijos_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbParametrosFijos_Vigencias] ON   [dbo].[tbParametrosFijos_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbParametrosFijos_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_prmtro_fjo
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbParametrosFijos_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbParametrosFijos y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbParametrosFijos_Vigencias  y  ' + @lcopcion_menu	
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbParametrosFijos_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbParametrosFijos_Vigencias]
ON  [dbo].[tbParametrosFijos_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbParametrosFijos_Vigencias ','cdgo_prmtro_fjo')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_prmtro_fjo = @lcCodigov  Or  cdgo_prmtro_fjo =  @lcCodigoi)
Begin
	RollBack Transaction
...
*---------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbParametrosFijos_Vigencias_cnsctvo_tpo_cncpto_intrs_mnsl]    ON [dbo].[tbParametrosFijos_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
DECLARE 	@lcmnsje	varchar(400)
...
If Not Exists (Select p.cnsctvo_cdgo_tpo_cntrto
			...
			from [bdAfiliacion].[dbo].[tbTiposContrato] p, Inserted i
			Where p.cnsctvo_cdgo_tpo_cntrto  = i.cnsctvo_tpo_cntrto_pc)
	Begin
		ROLLBACK TRANSACTION
		select @lcmnsje  =  'Error:,  Violación de Integridad Referencial con bdAfiliacion..tbTiposContrato'
		RAISERROR(999902, 16, 1,  @lcmnsje)
		RETURN
	End
End
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbParametrosFijos_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbParametrosFijos_Vigencias] ON   [dbo].[tbParametrosFijos_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		...
		@Codigo_Consecutivo = cnsctvo_cdgo_prmtro_fjo	,
		@cdgo_ara = cdgo_prmtro_fjo		,
		@cdgo_vgnca	= cnsctvo_vgnca_prmtro_fjo,
		@dscrpcn =  dscrpcn_prmtro_fjo
FROM   INSERTED
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_prmtro_fjo =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_prmtro_fjo !=  @cdgo_vgnca	
...
update tbParametrosFijos set dscrpcn_prmtro_fjo =  @dscrpcn
where cnsctvo_cdgo_prmtro_fjo = @Codigo_Consecutivo
