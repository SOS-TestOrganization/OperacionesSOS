CREATE TABLE [dbo].[tbPromotores] (
    [cnsctvo_cdgo_prmtr] [dbo].[udtConsecutivo] NOT NULL,
    [cdgo_prmtr]         [dbo].[udtCodigo]      NOT NULL,
    [dscrpcn_prmtr]      [dbo].[udtDescripcion] NOT NULL,
    [fcha_crcn]          DATETIME               NOT NULL,
    [usro_crcn]          [dbo].[udtUsuario]     NOT NULL,
    [vsble_usro]         [dbo].[udtLogico]      NOT NULL,
    CONSTRAINT [PK_TbPromotores_1] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_prmtr] ASC),
    CONSTRAINT [IX_tbPromotores] UNIQUE NONCLUSTERED ([cdgo_prmtr] ASC)
);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbPromotores
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbPromotores \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbPromotores] ON    [dbo].[tbPromotores]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbPromotores'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_prmtr,
	@cdgo = cdgo_prmtr,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_prmtr from tbPromotores
	where  cnsctvo_cdgo_prmtr = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbPromotores', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbPromotores_vigencias where  cnsctvo_cdgo_prmtr = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbPromotores where  cnsctvo_cdgo_prmtr = @cnsctvo_tbla
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbPromotores
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbPromotores]
ON  [dbo].[tbPromotores]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbPromotores','cdgo_prmtr')) - 1
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbPromotores
* Desarrollado por	: <\A Ing. Jorge Meneses  A\>
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbPromotores_usro_crcn]    ON    [dbo].[tbPromotores]
FOR  INSERT,UPDATE
AS
Begin
