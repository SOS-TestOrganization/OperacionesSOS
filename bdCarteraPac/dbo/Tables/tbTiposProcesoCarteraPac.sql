CREATE TABLE [dbo].[tbTiposProcesoCarteraPac] (
    ...
    CONSTRAINT [PK_tbTiposProcesoCarteraPac] PRIMARY KEY CLUSTERED ([cnsctvo_cdgo_tpo_prcso] ASC)
);


GO
CREATE STATISTICS [cdgo_tpo_prcso]
    ON [dbo].[tbTiposProcesoCarteraPac]([cdgo_tpo_prcso], [cnsctvo_cdgo_tpo_prcso]);


GO
CREATE STATISTICS [dscrpcn_prcso]
    ON [dbo].[tbTiposProcesoCarteraPac]([dscrpcn_prcso], [cnsctvo_cdgo_tpo_prcso]);


GO
CREATE STATISTICS [fcha_crcn]
    ON [dbo].[tbTiposProcesoCarteraPac]([fcha_crcn], [cnsctvo_cdgo_tpo_prcso]);


GO
CREATE STATISTICS [usro_crcn]
    ON [dbo].[tbTiposProcesoCarteraPac]([usro_crcn], [cnsctvo_cdgo_tpo_prcso]);


GO
CREATE STATISTICS [vsble_usro]
    ON [dbo].[tbTiposProcesoCarteraPac]([vsble_usro], [cnsctvo_cdgo_tpo_prcso]);
...
/*---------------------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaBorrar_tbTiposProcesoCarteraPac
...
* Descripcion			 : <\Borre las incidencias , las Vigencias e Insertar en tabla tbTiposProcesoCarteraPac \>
* Nuevos Parametros	 	 : <\PM   PM\>
* Nuevas Variables			 : <\VM   VM\>
* Fecha Modificacion		 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[trValidaBorrar_tbTiposProcesoCarteraPac] ON    [dbo].[tbTiposProcesoCarteraPac]
FOR DELETE
AS 
Declare   
...
where tbla_prmtro = 'tbTiposProcesoCarteraPac'
 
-- >Obtener los campos del parámetro  que se estan tratando de borrar
select    @cnsctvo_tbla =  cnsctvo_cdgo_tpo_prcso,
	@cdgo = cdgo_tpo_prcso,
...
--> Obetener la descripción de la Tabla Parámetro
	select @dscrpcn	=	dscrpcn_prcso from tbTiposProcesoCarteraPac
	where  cnsctvo_cdgo_tpo_prcso = @cnsctvo_tbla

--> Insertar el la tabla  tbParametrosEliminados
insert into bdCarteraPac..tbParametrosEliminados values (@liConsecutivo,'bdCarteraPac','tbTiposProcesoCarteraPac', @usro,@fcha_elmncn,@cnsctvo_tbla,@cdgo,@dscrpcn,null)

--> Borrar todos los Registros (vigencias, incidencias y parámetro) relacionados con el parámetro eliminado	
delete  tbTiposProcesoCarteraPac_vigencias where  cnsctvo_cdgo_tpo_prcso = @cnsctvo_tbla
delete tbIncidenciasCarteraPac  where cnsctvo_rgstro_afctdo = @cnsctvo_tbla
delete tbTiposProcesoCarteraPac where  cnsctvo_cdgo_tpo_prcso = @cnsctvo_tbla
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbTiposProcesoCarteraPac
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbTiposProcesoCarteraPac_usro_crcn]    ON    [dbo].[tbTiposProcesoCarteraPac]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbTiposProcesoCarteraPac
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbTiposProcesoCarteraPac]
ON  [dbo].[tbTiposProcesoCarteraPac]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbTiposProcesoCarteraPac','cdgo_tpo_prcso')) - 1
