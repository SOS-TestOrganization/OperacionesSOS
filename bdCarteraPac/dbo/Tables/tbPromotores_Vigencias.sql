CREATE TABLE [dbo].[tbPromotores_Vigencias] (
    [cnsctvo_vgnca_prmtr]       [dbo].[udtConsecutivo]               NOT NULL,
    [cnsctvo_cdgo_prmtr]        [dbo].[udtConsecutivo]               NOT NULL,
    [cdgo_prmtr]                [dbo].[udtCodigo]                    NOT NULL,
    [dscrpcn_prmtr]             [dbo].[udtDescripcion]               NULL,
    ...
    [nmro_idntfccn_prmtr]       [dbo].[udtNumeroIdentificacionLargo] NOT NULL,
    ...
    [cnsctvo_cdgo_tpo_prmtr]    [dbo].[udtConsecutivo]               NOT NULL,
    [prmr_aplldo]               [dbo].[udtApellido]                  NULL,
    [sgndo_aplldo]              [dbo].[udtApellido]                  NULL,
    [prmr_nmbre]                [dbo].[udtNombre]                    NULL,
    [sgndo_nmbre]               [dbo].[udtNombre]                    NULL,
    [dgto_vrfccn]               INT                                  NULL,
    [rzn_scl]                   VARCHAR (200)                        NULL,
    [cnsctvo_cdgo_clse_empldr]  [dbo].[udtConsecutivo]               NULL,
    [tme_stmp]                  ROWVERSION                           NOT NULL,
    CONSTRAINT [PK_TbPromotores] PRIMARY KEY CLUSTERED ([cnsctvo_vgnca_prmtr] ASC),
    CONSTRAINT [CKFinVigenciaMayorInicioVigencia_tbPromotores_Vigencias] CHECK ([fn_vgnca] >= [inco_vgnca]),
    CONSTRAINT [FK_tbPromotores_Vigencias_tbEstadosPromotor] FOREIGN KEY ([cnsctvo_cdgo_estdo_prmtr]) REFERENCES [dbo].[tbEstadosPromotor] ([cnsctvo_cdgo_estdo_prmtr]),
    CONSTRAINT [FK_tbPromotores_Vigencias_tbPromotores] FOREIGN KEY ([cnsctvo_cdgo_prmtr]) REFERENCES [dbo].[tbPromotores] ([cnsctvo_cdgo_prmtr]) ON DELETE CASCADE,
    CONSTRAINT [FK_tbPromotores_Vigencias_tbTiposPromotor] FOREIGN KEY ([cnsctvo_cdgo_tpo_prmtr]) REFERENCES [dbo].[tbTiposPromotor] ([cnsctvo_cdgo_tpo_prmtr])
);
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaTraslapeCodigo_tbPromotores_Vigencias
...
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaTraslapeCodigo_tbPromotores_Vigencias] ON   [dbo].[tbPromotores_Vigencias]
FOR  INSERT
AS
Begin
...
SELECT	@inco_vgnca = inco_vgnca,
		@fn_vgnca	= fn_vgnca,
		@Codigo_Consecutivo = cnsctvo_cdgo_prmtr	,
		@cdgo_ara = cdgo_prmtr		,
		@cdgo_vgnca	= cnsctvo_vgnca_prmtr,
		@dscrpcn =  dscrpcn_prmtr
FROM   INSERTED

	
-->Se Calcula si hay  traslape de fechas
	SELECT @contador = count(*)
	FROM  tbPromotores_Vigencias p
	...
	And	p.cnsctvo_cdgo_prmtr = @Codigo_Consecutivo
...
-->Para Indicar al usuario contra que fecha hay traslape
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_prmtr =  @Codigo_Consecutivo
		and 	cnsctvo_vgnca_prmtr !=  @cdgo_vgnca	
...
update tbPromotores set dscrpcn_prmtr =  @dscrpcn
where cnsctvo_cdgo_prmtr = @Codigo_Consecutivo
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrIncidencias_tbPromotores_Vigencias
* Desarrollado por	: <\A Ing. Maribel Valencia Herrera				 A\>
* DescripciÃ³n		: <\D Inserta un registro en la tabla tbInsidenciasCarterapac 		D\>
* 			  <\D Cuando se actualiza un Campo de tbPromotores_Vigencias  D\>
* Observaciones       	: <\O  O\>
* Parametros		: <\P   P\>
...
CREATE TRIGGER [dbo].[TrIncidencias_tbPromotores_Vigencias]  On  [dbo].[tbPromotores_Vigencias]
For  UPDATE  As
...
--	set @inicio = GETDATE()
	...
	SET @nombre_tabla	= 'bdCarteraPac.DBO.tbPromotores_Vigencias'
	SET @mascara_1_64 	=0x0000000030000000      --de derecha a izquierda: 0010000000000000010000001011100001111101000010000000000000100000
	SET @mascara_128_65 	= 0x0000000000000000      --de izquierda a derecha
	SET @mascara_combinada	= 0x30000000      
			...
			BEGIN
				...
				BEGIN
					...
					IF @id_columna = 5
						...
						)SELECT 'bdCarteraPac', 'tbPromotores_Vigencias',@lcCampo,
						d.cnsctvo_vgnca_prmtr, d.inco_vgnca, n.inco_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_prmtr
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_prmtr = n.cnsctvo_cdgo_prmtr AND 
							(
							 (
					...
					ELSE 	IF @id_columna = 6
						...
						d.cnsctvo_vgnca_prmtr, d.fn_vgnca, n.fn_vgnca,
						@lcUsuario,  @lcFechaIncidencia , d.cnsctvo_cdgo_prmtr
							FROM 	deleted d, inserted n 
						 WHERE 	d.cnsctvo_cdgo_prmtr = n.cnsctvo_cdgo_prmtr AND 
							(
...
/*---------------------------------------------------------------------------------
* Nombre		: TrValidaUpdateTraslapeCodigo_tbPromotores_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrValidaUpdateTraslapeCodigo_tbPromotores_Vigencias] ON   [dbo].[tbPromotores_Vigencias]
FOR  UPDATE
AS
Begin
...
SELECT     @dinco_vgnca   =  inco_vgnca,
	      @dfn_vgnca     =  fn_vgnca,
     	      @dcnsctvo_tbla  =  cnsctvo_cdgo_prmtr,
	      @cnsctvo_vgnca = cnsctvo_vgnca_prmtr			     
FROM	deleted
...
if  Exists (select * from #tbValoresAnteriores d,  #tbValoresNuevos n
              ...
              and d.cnsctvo_vgnca_prmtr = n.cnsctvo_vgnca_prmtr )
...
-->Hay traslape de fechas
	Begin
		...
		And	convert(varchar(10),p.fn_vgnca,111)     >= convert(varchar(10),@fn_vgnca,111)))  
		And	p.cnsctvo_cdgo_prmtr = @Codigo_Consecutivo
  	      	and       p.cnsctvo_vgnca_prmtr != @cnsctvo_vgnca
...
/*---------------------------------------------------------------------------------
* Nombre                 	: TrReferencia_tbPromotores_Vigencias
...
* Fecha Modificacion	: <\FM  FM\>
-----------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbPromotores_Vigencias_usro_crcn]    ON    [dbo].[tbPromotores_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
...
GO
/*--------------------------------------------------------------------------------------------------------
*Metodo o Programa	: 	TrValidaModifiVigenciasRelacion_ttbPromotores_Vigencias
* Desarrollado por	:  <\A  	  Ing. Maribel Valencia Herrera			A\>
* Descripcion		:  <\D   Valida la modificiacion  de fechas de vigencia en la tabla Parámetro	 D\>
			   <\D	tbPromotores_Vigencias contra las tablas de relación			D\>
* Observaciones		:  <\O			O\>
* Parametros		 :  <\P    		P\>
...
* Fecha Modificacion	 : <\FM   FM\>
*---------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrValidaModifiVigenciasRelacion_tbPromotores_Vigencias] ON   [dbo].[tbPromotores_Vigencias]
FOR UPDATE
AS 
Declare   
...
where tbla_prmtro='tbPromotores_Vigencias'

--> Se capturan las variables que se estan tratando de insertar
select  	@fcha_inco_vgnca = inco_vgnca,
	@fcha_fn_vgnca = fn_vgnca,
	@cnsctvo_tbla =  cnsctvo_cdgo_prmtr
from INSERTED
...
--> Validar  si las vigencias modificadas afectan alguna relacion existente con la tabla  tbPromotores_Vigencias
...
--> Armar la cadena para saber si existe alguna relacion en la tabla Relacion con el  codigo de la  tabla tbPromotores y las fechas modificadas
...
--> Si  se encuentra en , por encima o por debajo del rango de la relacion no  actualiza e indica con cual opcion del menú esta relacionada el parámetro
			...
			If      @lncontador=0
				 ...
				 ROLLBACK TRANSACTION
	     			 select @mnsje  =  'Error, Existe una  Relacion entre los Parámetros tbPromotores_Vigencias  y  ' + @lcopcion_menu	
...
GO
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------
* Nombre		: TrCodigosEspeciales_tbPromotores_Vigencias
* Desarrollado por	: <\A Ing. Jorge Meneses					A\>
...
* Fecha Modificacion	: <\FM  									FM\>
---------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
CREATE TRIGGER [dbo].[TrCodigosEspeciales_tbPromotores_Vigencias]
ON  [dbo].[tbPromotores_Vigencias]
FOR  UPDATE, DELETE 
AS 
...
Select	@lcLongitud = (COL_LENGTH('tbPromotores_Vigencias ','cdgo_prmtr')) - 1
...
If	Exists (	Select * From Deleted 
		Where cdgo_prmtr = @lcCodigov  Or  cdgo_prmtr =  @lcCodigoi)
Begin
	RollBack Transaction
...
*---------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbPromotores_Vigencias_cnsctvo_cdgo_tpo_idntfccn]    ON [dbo].[tbPromotores_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
DECLARE 	@lcmnsje	varchar(400)
...
* Modificado		:  Ing. Javier Paez
*---------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbPromotores_Vigencias_cnsctvo_cdgo_cdd]    ON [dbo].[tbPromotores_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
DECLARE 	@lcmnsje	varchar(400)
...
If Exists(Select 1 From Inserted)
Begin 
If Not Exists (Select p.cnsctvo_cdgo_cdd

			from [bdAfiliacion].[dbo].[tbCiudades] p, Inserted i
			Where p.cnsctvo_cdgo_cdd  = i.cnsctvo_cdgo_cdd )
	Begin
		ROLLBACK TRANSACTION
		select @lcmnsje  =  'Error:,  Violación de Integridad Referencial con bdAfiliacion..tbCiudades'
		RAISERROR(999902, 16, 1,  @lcmnsje)
		RETURN
	End
End
...
* Modificado		:  Ing. Javier Paez
*---------------------------------------------------------------------------------*/
CREATE TRIGGER  [dbo].[TrReferencia_tbPromotores_Vigencias_cnsctvo_cdgo_clse_empldr]    ON [dbo].[tbPromotores_Vigencias]
FOR  INSERT,UPDATE
AS
Begin
DECLARE 	@lcmnsje	varchar(400)
...
If Exists(Select 1 From Inserted)
Begin 
If Not Exists (Select p.cnsctvo_cdgo_clse_empldr

			from [bdAfiliacion].[dbo].[tbClasesEmpleador] p, Inserted i
			Where p.cnsctvo_cdgo_clse_empldr  = i.cnsctvo_cdgo_clse_empldr )
	Begin
		ROLLBACK TRANSACTION
		select @lcmnsje  =  'Error:,  Violación de Integridad Referencial con bdAfiliacion..tbClasesEmpleador'
		RAISERROR(999902, 16, 1,  @lcmnsje)
		RETURN
	End
End
